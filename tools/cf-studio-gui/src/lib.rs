...
/// The step-5 note about what the output folder holds **besides** the latest
...
pub fn format_stale_parts(provenance: Option<&RunProvenance>) -> Option<String> {
    ...
    let mut s = format!(
        "⚠ {n} part(s) in this folder were NOT regenerated by the latest cast \
...
";
    ...
    fn a_current_folder_says_nothing() {
        ...
        assert_eq!(format_stale_parts(Some(&current)), None);
    }
    ...
    fn the_note_names_each_part_and_distinguishes_unknown_from_older() {
        ...
        let note = format_stale_parts(Some(&provenance)).expect("two are stale");
