...
pub fn run_sliding_insertion_ramp(
    ...
    for k in 0..n_steps {
        ...
        // CR.0+ one-shot probe — per `docs/SIM_ARC_SLIDING_INTRUDER_CONTACT_RECON.md`
        // §5 row CR.0+. Bucket the raw `transformed.eval` over rest BCC vertices
        // at step 0 to empirically validate the planned `interior_cutoff =
        // 2 × cavity_inset_m = 6 mm` cutoff. Composed sd seen by `active_pairs`
        // = raw eval + cavity_inset_m (here +3 mm); interior_cutoff filter at the
        // active-set walk excludes composed < -6 mm ⇔ raw < -9 mm. REMOVED in CR.1.
        if k == 0 {
            let transformed = TransformedSdf::new(intruder.clone(), pose);
            let mut bins = [0usize; 7];
            let mut min_eval = f64::INFINITY;
            let mut max_eval = f64::NEG_INFINITY;
            for p in &rest_positions {
                let sd = transformed.eval(Point3::from(*p));
                if sd < min_eval {
                    min_eval = sd;
                }
                if sd > max_eval {
                ...
                let bin = if sd >= INSERTION_CONTACT_DHAT {
                    0
                } else if sd >= 0.0 {
                    1
                } else if sd >= -3.0e-3 {
            ...
            eprintln!(
                "[CR.0+ probe] step-0 histogram of raw transformed.eval over {} BCC verts:",
                rest_positions.len(),
            );
            eprintln!("  bin 0  eval >= +1 mm                : {}", bins[0]);
            eprintln!("  bin 1  eval ∈ [ 0  , +1 mm)         : {}", bins[1]);
            eprintln!("  bin 2  eval ∈ [-3 mm,  0  )         : {}", bins[2]);
            eprintln!("  bin 3  eval ∈ [-6 mm, -3 mm)        : {}", bins[3]);
            eprintln!("  bin 4  eval ∈ [-10 mm, -6 mm)       : {}", bins[4]);
            eprintln!("  bin 5  eval ∈ [-20 mm, -10 mm)      : {}", bins[5]);
            eprintln!("  bin 6  eval <  -20 mm               : {}", bins[6]);
            eprintln!(
                "  range: min = {:.3} mm, max = {:.3} mm",
                min_eval * 1.0e3,
                max_eval * 1.0e3,
            );
            // Composed sd seen by active_pairs = raw + 3 mm (cavity_inset_m).
            // Active-set band-gate: composed < d̂ = 1 mm ⇔ raw < -2 mm.
            // v5 interior_cutoff = 6 mm filters composed < -6 mm ⇔ raw < -9 mm.
            let n_active_no_cutoff = bins[2] + bins[3] + bins[4] + bins[5] + bins[6];
            let n_filtered_by_v5 = bins[5] + bins[6];
            let n_kept_by_v5 = bins[2] + bins[3] + bins[4];
            eprintln!(
                "  active-set (raw < -2 mm, no cutoff) : {} pairs",
                n_active_no_cutoff,
            );
            eprintln!(
                "  v5 cutoff (raw < -9 mm)             : filters ~{} pairs (≈bins 5+6); keeps ~{} pairs (≈bins 2+3+4 — straddles at -9 mm)",
                n_filtered_by_v5, n_kept_by_v5,
            );
        }
