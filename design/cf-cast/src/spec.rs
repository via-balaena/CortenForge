//! [`CastSpec`] — the multi-layer public API surface behind v2/v2.1's
...
/// this subdir when pointed at the cast dir.
///
...
/// silicone shell.
///
/// Mold-cup composition: [`CastSpec::export_molds_v2`] carves
...
///
/// [`Self::export_molds_v2`] casts each layer independently
/// against its own per-layer plug (layer 0 uses [`Self::plug`];
/// layer N>0 uses `layers[N-1].body`).
///
...
pub struct CastSpec {
    ...
    /// least one entry; `export_molds_v2` returns
...
pub struct V2LayerReport {
    ...
    /// Per-layer pour-volume + mass result, budget-gated before
    /// any STL is written.
...
/// lands on disk (pre-write atomicity).
#[derive(Debug, Clone)]
...
/// coordinates.
///
/// Surfaces enough post-export geometry for caller assertions
/// without retaining the whole mesh.
#[derive(Debug, Clone)]
...
impl CastSpec {
    ...
    /// budget without paying the meshing + F4-gate cost.
    /// [`Self::export_molds_v2`] and [`Self::write_procedure_v2`]
    ...
    ///   [`Self::export_molds_v2`]'s plug meshing step instead.)
    ...
    /// for this cast + `ribbon` to `path`. Renders v2-specific
    ...
    /// in `out_dir`.
    ///
    /// Each layer's mold cup is split into 2 pieces along the
    /// curve-following ribbon.
    ///
    /// Pipeline:
    ...
    /// running the F4 gate on each piece's mesh.
    ///
    /// **Atomicity**: curve-rotation gate, budget gate, meshing, and F4 gate all
...
///
/// Shared by [`CastSpec::export_molds_v2`] and
/// [`CastSpec::write_procedure_v2`] so the budget-enforcement contract
...
mod tests {
    ...
    use super::{CastError, CastLayer, CastSpec, CastTarget, MeshSummary};
    use crate::material::MoldingMaterial;
    ...
    fn baseline_cuboid_self_intersection_noise_floor() {
        ...
        let baseline_mesh = solid_to_mm_mesh(&plain_cuboid, 0.012, CastTarget::Platform).unwrap();
        ...
        assert_eq!(
            super::blocking_critical_count(&baseline_validation, CastTarget::Platform),
            0,
    ...
    fn compute_pour_volumes_errors_when_layers_is_empty() {
        // Standalone empty-layers gate: the pour-volume path must
        // fail before any SDF sampling.
        let spec = CastSpec {
            layers: Vec::new(),
