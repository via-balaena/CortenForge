//! Stage 1 integration smoke: build a `CastSpec` against a known-simple
//! body + capsule plug + cuboid bounding region, run `export_molds`,
//! and verify two non-empty STL files land on disk in `CARGO_TARGET_TMPDIR`.
//!
...
use cf_cast::CastSpec;
use cf_design::Solid;
use mesh_printability::PrinterConfig;
use nalgebra::Vector3;

/// Build the Stage 1 reference geometry — kept inline (rather than
/// pulling from `cf-cast`'s private `tests` module) so the integration
/// boundary is honest.
///
...
/// volume detector. Stage 2's real device geometry will use the same
...
#[test]
fn stage_1_export_molds_writes_two_stls() {
    let out_dir: PathBuf = [env!("CARGO_TARGET_TMPDIR"), "cf-cast-stage-1"]
        .iter()
        .collect();
    ...
    let report = spec
        ...
        .expect("Stage 1 export should succeed on reference geometry");

    // Files exist, non-empty. Binary-STL size formula:
    // 80-byte header + 4-byte face count + 50 bytes per triangle.
    let mold_meta = std::fs::metadata(&report.mold_path).expect("mold.stl should exist");
    let plug_meta = std::fs::metadata(&report.plug_path).expect("plug.stl should exist");
    let expected_mold_size = 84 + (report.mold_summary.face_count as u64) * 50;
    ...
    assert!(
        !report.mold_summary.aabb_mm.is_empty(),
        "mold AABB populated"
    );
    assert!(
    ...
    let mold_z_min = report.mold_summary.aabb_mm.min.z;
    let mold_z_max = report.mold_summary.aabb_mm.max.z;
    assert!(
    ...
    println!("mold STL: {}", report.mold_path.display());
    println!("plug STL: {}", report.plug_path.display());
    println!(
        "mold: {} verts, {} faces, bounds {:?}",
        report.mold_summary.vertex_count,
        report.mold_summary.face_count,
        report.mold_summary.aabb_mm
    );
    println!(
