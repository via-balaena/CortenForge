//! Benchmarks for mesh-remesh operations.
//!
//! Run with: cargo bench -p mesh-remesh
//!
    ...
    for (name, mesh) in &test_cases {
        ...
        group.bench_with_input(BenchmarkId::new("isotropic", name), mesh, |b, mesh| {
            let params = RemeshParams::default()
                .with_target_length(0.1)
                .with_iterations(3);
            b.iter(|| remesh(black_box(mesh), black_box(&params)));
        });

        group.bench_with_input(BenchmarkId::new("high_quality", name), mesh, |b, mesh| {
            let params = RemeshParams::high_quality().with_target_length(0.1);
            b.iter(|| remesh(black_box(mesh), black_box(&params)));
        });
    }
...
criterion_group!(benches, bench_remeshing);
