...
- Single cross-section at `z = 5.0` (mid-cylinder):
  ...
  - `centroid` empirically `≈ (-0.0762, -0.0075, 5.0)` within `1e-12` — **NOT** the polygon centroid `(0, 0, 5)`. The library returns `sum / N` over chain-closure-duplicated points (`points.len() = 65`: 64 unique perimeter points + 1 chain-closure duplicate). For our symmetric 32-gon perimeter, `sum_unique = 0`, so `centroid = V_dup / 65` where `V_dup` is the duplicated chain-closure vertex; probe shows `V_dup = mid_(16, 17)` at z=5, giving `centroid = (-(1+cos(π/16))/26, -sin(π/16)/26, 5)`. Drift surfaced 2026-05-02 at §6.2 #6 execution; resolution: lock empirical biased value, surface as v0.9 candidate (§10 item 11). Anchor literal value held to `1e-12` (deterministic per FP).
  - `contour_count == 1`.
...
- Slice stack via `cross_sections(start=(0,0,0.5), normal=(0,0,1), count=10, spacing=1.0)`:
  - 10 slices returned.
  - All 10 have `area = 400·sin(π/16) ≈ 78.0357` within `1e-10` (polygon shoelace, **NOT** analytical π·r²; same chord-shrinkage anchor as the single-slice). Drift-8 fix 2026-05-02: previous draft asserted "0.5% of analytical" which fails by 0.64% chord-shrinkage; bringing slice-stack into line with the cold-read's single-slice fix per HE-2.
  - All 10 have `perimeter = 320·sin(π/32) ≈ 31.3655` within `1e-10` (z-uniform cylinder; polygon shape is invariant in z because the per-side chord-interior point lies on the chord at parameter z/h).
  - Each slice's `plane_origin.z` matches the analytical `0.5 + i·1.0`.
- Convenience helpers:
...
- **Bulk query**: 1000-point cubic grid in `[-2, 2]³` (10 × 10 × 10, spacing `4/9`, endpoint-inclusive at `-2` and `2`) cached as `Vec<(Point3<f64>, f64)>`. **Stats printed (drift-9 + drift-10 corrections, applied 2026-05-02 inline at §6.2 #10)**: TWO inside-counters disagree by design — `point_in_mesh` (ray-casting) reports `inside_count == 8` matching the analytical L1-ball (8 grid points at `(±2/9, ±2/9, ±2/9)` with `|x|+|y|+|z| = 6/9 < 1`; next-nearest sum `10/9 > 1` is exterior); `signed_distance < 0` (face-normal-of-closest-face sign convention) reports `inside_count == 14` — 8 true + 6 vertex-region FALSE-POSITIVES at the bbox boundary (drift-10). Continuous-volume sanity check: octahedron volume is `(4/3)·r³` (L1-ball; drift-9 — previous draft asserted `(8/3)·r³`, off by factor 2), giving continuous fraction `(4/3)/64 = 1/48 ≈ 2.083%`. Max unsigned distance `(2+2+2)/√3 - 1/√3 ≈ 5/√3 ≈ 2.887` achieved at all 8 bbox corners (e.g., `(-2,-2,-2)` projects to `(-1/3,-1/3,-1/3)` on `f7`'s plane).
...
- After `generate_lattice`:
  ...
  - `result.actual_density` is finite and in `[0.0, 1.0]`. **Drift-12** (corrected 2026-05-02 inline at this commit): the prior anchor declared "approximate range `[0.3, 0.7]` for density 0.5 input" — empirically the value is `~0.06`, off by ~8× from the expected ~50% wall-volume fraction. F9 is more broken than the spec assumed; `estimate_mesh_volume` (`generate.rs:552-576`) is the divergence-theorem formula `(1/6)|Σ v0·(v1×v2)|`, which requires a closed orientable manifold mesh with consistently-oriented triangles. The un-welded MC output for shell topologies (two boundary components at `|G| = 0.75`) violates one or more of those preconditions — exact cause untraced (candidates: TRI_TABLE orientation inconsistencies between cube cases, `is_degenerate` filter interactions, or seam handling between adjacent cells). The heuristic cannot be range-anchored on shell topologies; v0.9 fix is proper volume integration on welded mesh, gated on F10 weld-pass.
...
- **Surface verification via SDF-on-output** — anchor that vertices live on the *shell wall* surface, NOT on the bare gyroid level set. **Drift-11** (corrected 2026-05-02 inline at this commit): the prior anchor `gyroid(v).abs() < threshold + voxel_size + epsilon` with `threshold = 0` for density 0.5 implied vertices live near `G = 0`. Geometric truth: `generate_tpms_lattice` (`generate.rs:379-386`) wraps gyroid into `make_shell`, placing the lattice surface at `|G| = half_thickness = 0.75` (NOT `G = 0`); MC extracts THAT surface. Reformulated anchor: `||gyroid(v, 10.0)| − half_thickness| < 0.05` for all output vertices. The `0.05` bound is empirically-locked (~2× the observed max ~0.023), and is 30× tighter than the analytical worst-case bound `½·|∇²G|_max·voxel² ≈ 0.18` because gyroid second-derivative magnitude is well below its theoretical max across most cells. Cushion absorbs cross-platform libm sin/cos drift. Spec's prior 1.0 cushion happened to absorb both `half_thickness` AND quantization, but the framing implied the wrong geometric model.
...
| 8 | SDF sign-convention upgrade (pseudo-normal / winding-number) at vertex / edge regions | mesh-sdf | **Trigger expanded 2026-05-02 by §5.4 example (drift-10)**: surfaced empirically — for the 1000-point bulk-grid scan against the unit octahedron (CONVEX), `signed_distance < 0` reports 14 inside while `point_in_mesh` (ray-casting) correctly reports 8. The 6 false-positives are at points like `(-2, -2/3, 2/3)` where 4 faces tie on the closest octahedron vertex `(-1, 0, 0)` and the strict-`<` tie-break in `SignedDistanceField::distance` picks a face whose outward normal `to_point.dot(normal)` flips negative even though the point is geometrically OUTSIDE. Original trigger phrasing (concave-mesh) was too narrow; failure applies to vertex / edge regions of ANY mesh, including CONVEX. Original trigger preserved: a user reports SDF sign flipping incorrectly near edges or vertices of any geometry | Current implementation uses face-normal-of-closest-face per `sdf.rs:172-185`; pseudo-normal (Bærentzen-Aanæs angle-weighted vertex normal) or winding-number-based sign is robust regardless of which closest-point region wins on tie-break | medium (~200 LOC; algorithm is well-documented in literature) |
...
| 11 | Proper polygon centroid in `CrossSection` (shoelace-weighted, not naive average) | mesh-measure | Surfaced 2026-05-02 by §5.2 example execution. Current `cross_section.rs:171-176` computes `centroid = sum / all_points.len()` — but `chain_segments` produces a contour with one chain-closure duplicate (`points.len() = N+1`), so for a symmetric polygon `centroid = V_dup / (N+1) ≠ (0, 0, 0)`. For our 32-segment cylinder mid-slice the bias is `~0.077` mm in (x, y); the existing `test_cross_section_centroid` (line 549) accommodates this by asserting `< 0.2` tolerance | Replace with shoelace-weighted polygon centroid: `c_x = (1/(6A))·Σ(x_i + x_(i+1))·(x_i·y_(i+1) − x_(i+1)·y_i)` and similar for `c_y` (project to 2D plane via the same `(u, v)` basis as `calculate_cross_section_area`, then back to 3D). Verify the existing `test_cross_section_centroid` tolerance can tighten from `0.2` → `1e-10` after the fix | small (~30 LOC: replace the `else` branch in centroid calc + lift `(u, v)` basis from area calc; tighten existing test) |
