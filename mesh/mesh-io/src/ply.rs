...
pub fn load_ply<P: AsRef<Path>>(path: P) -> IoResult<IndexedMesh> {
    ...
    // Parse header
    let header = parser.read_header(&mut reader).map_err(|e| {
        IoError::invalid_content(format!("failed to parse PLY header: {e}"))
    })?;

    // Read the entire payload
    let payload = parser.read_payload(&mut reader, &header).map_err(|e| {
        IoError::invalid_content(format!("failed to read PLY payload: {e}"))
    })?;
...
fn save_ply_ascii<W: std::io::Write>(mesh: &IndexedMesh, writer: &mut W) -> IoResult<()> {
    ...
    ply.header.comments.push("Generated by CortenForge mesh-io".to_string());
    ...
    ply_writer.write_ply(writer, &mut ply).map_err(|e| {
        IoError::invalid_content(format!("failed to write PLY: {e}"))
    })?;
