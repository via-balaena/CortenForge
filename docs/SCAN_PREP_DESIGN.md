...
- **2026-05-12** — Commit #5 (Simplify panel + meshopt-rs integration) ships (this commit). Two spec amendments folded in: (a) §1 Scan Info wording "Loaded at startup, never changes" → "Static fields (file path, STL units, raw AABB) loaded at startup; vertex/face counts update when the Simplify panel applies a decimation pass (commit #5)" — the original "never changes" wording predated the Simplify panel's design and is now factually wrong since `[Apply simplify]` mutates the displayed vertex/face counts; (b) §2 Simplify example string `"Reduced 3.35M → 198,432 faces in 4.2s"` → `"Reduced 3.35M -> 198,432 faces in 4.2s"` (same egui ProggyClean `→`-glyph constraint as commit #4's STL units fix; the live status bar emits ASCII `->`). New workspace dep `meshopt = "0.6.2"` (boundary-preserving quadric edge collapse via FFI to `meshoptimizer`); new cf-scan-prep dep `mesh-repair` (`weld_vertices` pre-simplify since STL load produces 3N unshared vertices that meshopt can't see as collapsible edges). `SimplifyOptions::LockBorder` flag pins open-boundary loop vertices in place — load-bearing for downstream Cap detection at commit #9 seeing the same loop topology after decimation. Implementation arc unchanged at 14 commits / ~28-37 hours; commit #5 actual ~4 hr inclusive of recon + spec amendments.
