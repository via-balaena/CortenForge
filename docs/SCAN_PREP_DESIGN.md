...
- **2026-05-12** — Commit #7 (Recenter panel) ships (`28837d17`). Three translation sliders + `[Center origin]` + `[Floor -> z=0]` + `[Reset translation]` buttons, mirroring Reorient's pattern. State stored in **physics-frame millimeters** (mm to match the AABB display convention; physics-frame because the saved STL bakes the translation into vertex positions directly at commit #12). Slider range per spec: `±2 × max(raw_aabb)` (mm) — scan-derived, pre-computed at startup in `build_overlay_lengths`. `[Center origin]` and `[Floor -> z=0]` are click-time computations: walk the 8 corners of the raw AABB through the current `ReorientState` quaternion, recompute the rotated bbox bounds in physics mm via new helper `rotated_aabb_physics_mm`, set slider values to the negation of (center / min.z). If the user changes rotation after clicking, the centering becomes stale — they re-click to re-center (spec confirms these are buttons, not continuous behaviors). The button label "Floor -> z=0" uses ASCII `->` matching the commit #4 / #6 glyph-coverage pattern. New `apply_recenter_to_transform` Update system mirrors `apply_reorient_to_transform` — writes `Transform.translation` per tick, idempotent equality-check, automatic re-apply after `handle_simplify_actions` respawn. `draw_reference_overlays` updated to compose translation + rotation in the wireframe's transform so the wireframe stays glued to the mesh through both Reorient and Recenter edits. 4 new tests pin RecenterState default, the physics→bevy translation projection (swap + scale), the `rotated_aabb_physics_mm` identity case, and the `rotated_aabb_physics_mm` 90°-about-X axis-extent swap. `OverlayLengths` gained two fields (`raw_aabb_m` cached for click-handler math; `recenter_slider_range_mm` for the slider extent); the existing overlay test extended to pin both. cf-scan-prep tests now 34; xtask grade A. No spec amendments. Implementation arc unchanged at 14 commits / ~28-37 hours; commit #7 actual ~1.5 hr inclusive of design preview + spec slice-log writeup.
- **2026-05-12** — Commit #8 (Clip floor + true plane intersection algorithm) ships (`16736463` + `2a66259e` cache fix). The triangle-clipping math + panel UI + visualization, plus the load-bearing strategic decision to pursue **curve-following multi-piece molds as the primary cf-cast architecture** (deferred to post-iter-1 per the agreed option-3 plan; design doc lands alongside commit #9's centerline visualization). New `ClipState { enabled, z_mm }` resource. New panel section with enable checkbox + Z slider (range mirroring Recenter: `±2 × max(raw_aabb_mm)`) + live `compute_drop_percentage` readout (vertex walk through Reorient rotation + Recenter Z, classifies each against world `clip_z_mm`; cheap for simplified meshes at ~1 ms per 200k verts). `draw_reference_overlays` gains a disc visualization when enabled: 4 corner lines + 2 diagonals forming a flat translucent-orange rectangle in the cast-frame XY plane at world `z = clip_z` (sized to `0.75 × max(bbox_dims_bevy)` for clear visual coverage). Auto-warning when drops >95% ("⚠ Clip removes nearly all geometry — verify Z value") catches above-mesh / inverted-axis user errors per spec edge case. **Algorithm `clip_mesh_against_world_z` shipped + unit-tested but sits dormant** — not yet applied to the displayed mesh to avoid respawn-race complexity with `handle_simplify_actions`. The algorithm gets exercised at commit #9 (Cap detection consumes the clipped mesh for boundary identification) and commit #12 (Save bakes the clip into the cleaned STL). True-plane-intersection algorithm walks each triangle CCW, classifies vertices against the world-z-plane projected to mesh-local frame, generates intersection vertices on crossing edges, fan-triangulates the surviving polygon (3 or 4 vertices). 9 new tests pin: ClipState default, drop-percentage on empty mesh, drop-percentage counts below-plane vertices, triangle entirely above kept, triangle entirely below dropped, 1-above-2-below produces 1 triangle with 2 intersection vertices on z=0 plane, 2-above-1-below produces 2 triangles with 2 intersection vertices, on-plane vertex stays whole (spec edge case), rotation correctly applied when computing the plane in mesh-local frame. cf-scan-prep tests now 43; xtask grade A. No spec amendments. Implementation arc unchanged at 14 commits / ~28-37 hours; commit #8 actual ~2.5 hr.
- **2026-05-12** — Simplify `target_error` calibration polish (`2a312efd`). Bumped `SIMPLIFY_TARGET_ERROR` from `0.01` to `0.05` after eyes-on-pixels showed the iter-1 fixture simplifying from 3.35M to 3.33M faces (~0.6% reduction; basically nothing) at the original threshold. Root cause: meshopt's `target_error` is normalized to mesh extents (1.0 = half the longest axis); at 0.01 on a 130mm-tall scan the tolerance was ~0.65mm — tighter than the sock-textile noise the scan was full of, so meshopt hit the error budget before reaching the requested face count. **The justifying anchor is cf-cast's 2mm SDF cell** (per spec §Strategic context): geometric detail below 2mm doesn't transfer to silicone anyway, so preserving sub-mm features costs nothing useful. 0.05 ≈ 3.25mm worst-case tolerance on the iter-1 scan — above the 2mm SDF floor + below typical scan-feature scales. Docstring rewritten to record the calibration rationale + the iter-1 history; recoverable by tightening back toward 0.01 if iter-1 cast surfaces visible surface artifacts. No behavior change for the algorithm itself; just a tuning constant. 43/43 tests still green.
- **2026-05-12** — Commit #9 (Cap open boundaries + centerline visualization) ships (this commit). The detection-and-visualization core of the Cap panel: boundary loop extraction (via `mesh_repair::detect_holes`), least-squares plane fit per loop (SVD on covariance matrix; nalgebra), outward-normal heuristic (mesh-side-of-plane majority voting), per-loop checkbox + R² readout + vertex count UI, stale-on-transform-change invalidation, and gizmo-linestrip overlays in the viewport. Plus the **option-3 hedge**: cross-section-centroid centerline polyline visualization, drawn as a cyan linestrip from base to tip — proves the centerline algorithm works on real scans BEFORE curve-following cf-cast commits to consuming it. **Scope deviation from spec**: ear-clipping triangulation (~200 LOC) deferred from commit #9 to commit #12 (Save panel), where the cap polygon actually gets baked into the cleaned STL. Visualization uses gizmo line-loops + outlines rather than translucent filled polygons (which would require triangulation); same information content for less LOC + matches Bevy's gizmo capabilities. `[Apply caps]` button deferred along with triangulation; user's per-loop include checkboxes serve as the configuration state that commit #12 reads. Cap-stale invalidation pattern matches spec §Panel specifications §6: any Reorient / Recenter / Simplify mutation after a scan marks `CapState::stale = true`; overlays dim to 35% brightness + the panel shows "⚠ Transform changed — re-Scan to refresh"; user re-clicks `[Scan]` to refresh. New resources: `DetectedCapLoop { vertex_indices, plane_centroid, plane_normal, plane_fit_r_squared, include }`, `CapState { loops, centerline_polyline, stale }`, `CapPendingAction { rescan }`. New systems: `handle_cap_actions` (consumes rescan trigger, runs detection on explicit user click rather than every state change — avoids per-frame O(N) detection cost), `mark_cap_stale_on_transform_change` (sets stale flag via `bypass_change_detection` to avoid self-loop), `draw_cap_overlays` (red/orange/yellow/magenta cycling palette for per-loop disambiguation, plus cyan centerline). Centerline algorithm choice: **cross-section centroids** (~100 LOC, deterministic, no iteration tuning) over the alternatives (mean curvature flow ~400-600 LOC + tuning-sensitive, Voronoi-of-surface-samples ~500 LOC + needs 3D Voronoi crate that doesn't exist in Rust). For mostly-tubular prosthetic appendages — the primary use case — cross-section centroids produce centerlines that follow the natural curve. Fails on branching geometry (multi-finger hands, multi-limb torsos); upgrade path to MCF is open if iter-1 surfaces that case. 6 new tests pin: plane fit on perfectly-planar XY loop → R² = 1.0 + ±Z normal; non-planar twisted loop → R² < 1.0; degenerate input → fallback; outward normal flips away from mesh-majority side; centerline on elongated +X mesh → polyline follows +X axis with y ≈ z ≈ 0; centerline degenerate input → empty Vec. cf-scan-prep tests now 49; xtask grade A. Implementation arc unchanged at 14 commits / ~28-37 hours; commit #9 actual ~3.5 hr.
- **2026-05-13** — Commit #12 (Save panel — cleaned STL + .prep.toml + ear-clipping cap triangulation) ships (this commit). Closes the MVP authoring loop: clicking `[Save]` bakes Reorient rotation + Recenter translation into vertex positions, triangulates each included cap loop via inline 2D ear-clipping (~200 LOC, with fan-fallback for degenerate / self-intersecting projections), appends the cap triangles to the mesh, and atomically writes `<stem>.cleaned.stl` (binary STL) + `<stem>.prep.toml` (provenance) to the configured output directory. **Scope cut per v2 pivot**: clip baking deliberately *not* applied at Save — the Clip panel stays advisory; v2 cf-cast handles trimming via the centerline-derived mold geometry rather than a pre-baked floor plane (the v1 "everything below z=floor → discard" model was the failure-mode that drove the v2 pivot in the first place). Commit #10 (Mouth ext.) + #11 (Cleaned AABB) are likewise dropped from the MVP arc: Mouth extension was a v1 prep affordance for straight-pull plug cavities (which v2's per-piece molds don't need), and the Cleaned AABB read-only summary is subsumed by the Save panel's filename previews + the always-on AABB wireframe. **New CLI flag**: `--output-dir <PATH>` honors an explicit destination; default falls back to the input scan's parent directory; final fallback to CWD if both are absent. New resources: `SavePendingAction`, `SaveOutputDir`, `SourceStlPath`, `StlUnitsResource`. New `StatusKind::Error` (red) variant for FS error surfacing — non-finite Reorient / Recenter values + disk-write failures surface as red status-bar messages with 6-8 second auto-clear rather than panicking. Atomic-write pattern: write both files to `.tmp` siblings first, then rename both; on either-step failure remove both temp files + roll back any landed final file so the user never ends up with a half-written save. Ear-clip path: project 3D loop to 2D via Gram-Schmidt orthonormal basis on the fit plane → signed-area sign check (auto-reverse CW → CCW) → O(n²) ear-finding loop with convex-corner + empty-ear validation → fan fallback on stuck states. Triangle winding picked so the cap's outward-pointing 3D normal matches `DetectedCapLoop::plane_normal` (which `build_detected_cap_loop` already flipped to face away from mesh interior). The `.prep.toml` schema records: scan_prep block (source path, tool version, ISO 8601 generation timestamp, STL units), reorient (quaternion + Euler degrees), recenter (translation in meters + mm), clip (enabled, z, drop_pct, `baked = false` for v2 transparency), caps (per-loop vertex count + R² + outward normal + centroid + included flag), centerline (post-bake world-frame meters polyline — directly consumable by v2 cf-cast), output (cleaned STL filename). Inline `iso8601_utc_from_unix_seconds` + `unix_days_to_ymd` (Howard Hinnant civil-from-days) avoid pulling in chrono for one timestamp. cf-scan-prep tests now 59 (+10 Save: ear-clip on unit square, CW polygon, degenerate vertex counts, convex pentagon; ISO 8601 known-epoch pin; .prep.toml TOML round-trip; build_cleaned_mesh recenter baking; resolve_output_dir explicit-flag + default-to-parent; atomic_write_save both-files-landed). xtask grade A. Implementation arc cut to 12 commits (#10 + #11 dropped); commit #12 actual ~3 hr.
