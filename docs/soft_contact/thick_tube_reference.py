...
It asserts its own checks (the incompressible limit, the Lamé limit, and
tolerance stability) before printing the reference tables.
"""
import numpy as np
...
def shoot(lr_A, A, B, a, lz, mu, lam, rtol, atol):
...
def solve(A, B, a, lz, mu, lam, rtol=1e-12, atol=1e-14):
    ...
    sol, _ = shoot(lr_A, A, B, a, lz, mu, lam, rtol, atol)
    lt_A = a / A
    J_A = lr_A * lt_A * lz
...
def solve_cased_free_ends(A, B, a, mu, lam):
    ...
    while not (np.isfinite(g(hi)) and g(hi) > 0):  # axial force turns tensile once lz is large enough
        hi = 1.0 + 2.0 * (hi - 1.0)
        assert hi < 1.6, "no bracket for lz"
    assert g(lo) < 0
...
def _checks():
    ...
    assert abs(haughton_ogden(A, 2.0, 1.3, 1.0, mu) - 0.31338) < 1e-5
    for lz in (1.0, 0.9):
        p = solve(A, 2.0, 1.3, lz, mu, lam_inc)[0]
        assert abs(p / haughton_ogden(A, 2.0, 1.3, lz, mu) - 1) < 1e-4, (lz, p)
    ...
    assert abs(lz - lz_inc) < 1e-5, (lz, lz_inc)
    ...
    assert abs(gaps[1]) < abs(gaps[0]) / 5, gaps
    # 3. Tolerance stability.
    lam = 2 * 0.495 / 0.01
    p10 = solve(A, 2.0, 1.3, 1.0, mu, lam, rtol=1e-10, atol=1e-12)[0]
    p12 = solve(A, 2.0, 1.3, 1.0, mu, lam)[0]
    assert abs(p10 / p12 - 1) < 1e-8, (p10, p12)
    print("checks passed: incompressible limit (lz = 1, 0.9, free ends: pressure and lz), Lamé limit, tolerance")
