...
writes, after the same checks, K2's corners and the confined case (plan §15a,
15d.8) as Rust constants: p/mu, lz, and the derivatives the gap-corrected
reference linearizes with (plan 15d.1). A regenerated file is a reviewed change.
"""
import numpy as np
...
# K2's corners and the confined case (plan §15a, 15d.8): (a/A, B/A, nu, walls).
GOLDEN_CASES = [
    (1.1, 2.0, 0.49, "Free"),
    (1.1, 2.0, 0.495, "Free"),
    (1.3, 2.0, 0.49, "Free"),
...
# The plan's own values for these cases (§15b's table, 15d.8), to 5 significant figures.
PLAN_VALUES = {
...
def golden_values(la, BA, nu, walls, h=1e-6):
    """p/mu, lz, d(p/mu)/d(a/A) at fixed lz, and d(p/mu)/dlz at fixed a, by central differences."""
    mu, A = 1.0, 1.0
    lam = 2 * nu / (1 - 2 * nu)
    if walls == "Free":
        p, lz = solve_free_ends(A, BA, la, mu, lam)
        at = lambda a, z: solve(A, BA, a, z, mu, lam)[0]
    else:
        lz = 1.0
...
def _rust_case(la, BA, nu, walls, values):
...
def write_golden(path):
    rows = []
    for la, BA, nu, walls in GOLDEN_CASES:
        values = golden_values(la, BA, nu, walls)
        ...
        rows.append(_rust_case(la, BA, nu, walls, values))
    text = (
