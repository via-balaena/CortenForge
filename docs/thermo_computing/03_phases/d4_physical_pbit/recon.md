...
- **Bath = injected mechanical noise.** A macroscopic element won't switch from
  ...
  **S2 finding (2026-06-05):** this calibration is only quantitative when the bit
  is *cold* (`kT_eff ≪ ΔV`, a single occupied well) — at the switching
  temperatures `kT ≈ ΔV` the measured `⟨δx²⟩` reads ~40% low, but as a measurement
  artifact (the analyser's barrier-side truncation gate), not anharmonicity (true
  single-well anharmonicity *enhances* `⟨δx²⟩`). So read `kT_eff` at low drive deep
  in one well, not while it's flipping. The
...
- **S3** — analysis/calibration tool. **Shipped in G1:** ingest the CSV →
  switching rate, transition events, dwell-time distribution, in-well `⟨δx²⟩`,
  single-vs-bimodal detection, and the `kt_eff` / `kramers_rate` physics helpers
  (library functions + drift-guard tests). **Deferred to the physical gates (G5,
  needs the rig + real captures):** the ring-down `γ` fit, the `(ΔV, x₀)` fit, the
  count→position calibration, and wiring those into a measured-vs-predicted
  overlay + sim-to-real gap. The helpers exist; the end-to-end overlay waits on
  hardware data.
...
- **S2:** **DONE** (`sim/L0/rl-baselines/tests/kramers_pbit.rs`). The sim reproduces
  ...
  is **−3.4 vs −ΔV=−3.0** (~15%; the residual is the Kramers formula's
  moderate-barrier correction — switches are only countable at ΔV/kT ~ 1–2, away
  from the deep-barrier limit where slope = −ΔV is exact). Dwell-time **CV ≈ 0.9**
