...
2. `check_basic_manifold` — emits `NotWatertight` / `NonManifold` Critical via undirected-edge incidence, plus a winding-orientation and a degenerate-face pass, both read from `mesh_repair::winding_census`. ⚠ Gap F catches **locally inconsistent** winding — neighbouring faces disagreeing across a shared edge — which v0.7 silently accepted. It does **not** catch an inside-out shell: a uniform reversal leaves every edge in agreement and is invisible to any per-edge test. `MeshReport::is_inside_out` is the (frame-dependent) instrument for that, and `check_basic_manifold` does not consult it.
...
1. **Cheap topology checks first.** `check_build_volume` runs in `O(V)` (one bbox pass) and `check_basic_manifold` runs in `O(F)` (edge-incidence plus the census). They are the gating checks that downstream detectors implicitly depend on — `check_thin_walls` and `check_trapped_volumes` skip with `DetectorSkipped` on malformed input, since their algorithms (inward ray-cast, voxel parity) need a closed-surface contract. Running the topology checks first means later detectors emit informative skip diagnostics rather than crashing.

   ⚠ **Their preconditions are not the same one**, and the difference is load-bearing. `check_thin_walls` requires watertight **and** manifold **and** consistently wound (`is_watertight_and_consistent_winding`). `check_trapped_volumes` requires only the absence of *open* edges (`is_watertight`, i.e. no edge with exactly one incident face) — it tolerates non-manifold edges and inconsistent winding, because voxel parity survives both where ray-casting does not. A mis-wound closed mesh therefore skips ThinWall while still running TrappedVolume; `stress_cross_inconsistent_winding_blocks_thin_wall_only` is the fixture that pins exactly this asymmetry.
