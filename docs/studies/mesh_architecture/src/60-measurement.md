...
The depth pass covers the algorithmic specifics (per-triangle plane intersection, edge-walking to assemble closed contours, handling of the degenerate cases where the plane grazes a vertex or coincides with a face) and the cross-section's role in soft-body visualization examples (the [`measure/stress-test`](80-examples.md) `cross_section` module walks through the planar-slicing surface end-to-end).
