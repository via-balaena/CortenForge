...
- **`sim-soft` does not ship a penalty contact implementation as a *production* baseline.** Phase 5 ships penalty as a deliberate, named-removed-at-Phase-H stepping stone for exactly the validation scope §00 §00 names: rigid↔soft co-sim plumbing, first-time `ContactModel` wiring into the Newton hot path, and an analytic-comparison scientific gate (Hertz). The penalty implementation lives at `sim/L0/soft/src/contact/penalty.rs` per Phase 5 scope memo Decision A. Penalty's structural failure modes documented above remain valid — gradient optimization through penalty is restricted to the robustly-in-contact band (Phase 5 invariant V-7 enforces this restriction); contact-pressure read into `RewardBreakdown` is best-effort with a documented overlap tolerance, not the precise observability IPC will provide at Phase H. The [Part 4 Ch 01 barrier](../01-ipc-internals/00-barrier.md) remains the only *production* contact force law.
