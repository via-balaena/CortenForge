...
Here $\epsilon_v$ is a velocity threshold in m/s, so $h\epsilon_v$ has units of displacement and serves as the width of the smooth transition zone $[0, h\epsilon_v]$ in displacement space. The IPC default is $\epsilon_v = 10^{-3} \ell$ m/s for bounding-box length $\ell$.
