...
The cost is ≈3–5× Tet4 per-element (more nodes, more Gauss points, denser stiffness submatrix), and the stiffness matrix has ≈2.5× the nonzero count for the same tet count. The net wall-clock penalty depends on what dominates — per-element assembly or per-DOF sparse solve — but Tet10's overhead stays in the range where the rim-deformation fidelity gain justifies it, which is why Phase H's fidelity targets demand it.
...
- [Part 11 Ch 00 element module](../110-crate/00-module-layout/01-element.md) ships Tet4 in Phase B and adds Tet10 in Phase H without changing the module's trait surface from [Part 11 Ch 01](../110-crate/01-traits.md).
