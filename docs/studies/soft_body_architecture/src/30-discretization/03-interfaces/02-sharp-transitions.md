...
If the workload requires a genuinely sharp transition (e.g., a sputter-deposited metallization layer), the smooth-blend treatment is unsuitable and a [bonded-interface](00-bonded.md) or [sliding-interface](01-sliding.md) treatment with explicit interface elements is the right answer. The choice is design-time, not runtime — the transition character is a property of the physical part and is reflected in the [`SdfField` composition](../../../70-sdf-pipeline/02-material-assignment/01-composition.md) the user authors.
