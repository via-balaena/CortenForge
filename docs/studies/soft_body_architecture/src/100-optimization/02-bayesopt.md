...
- **Trust-region BayesOpt (TuRBO).** Maintain a local trust region around the best-seen point, run BayesOpt inside it, expand or contract based on progress. Reaches input dimensions where classical GP-BayesOpt no longer converges, with strong empirical performance on non-convex benchmarks drawn from robotics, reinforcement learning, and scientific applications (Eriksson et al. 2019). Phase H scope.
