...
`sim-soft`'s Phase I commits to **diffuse-profile screen-space SSS**, the middle tier. The cost fits the Phase I frame budget on a consumer GPU at 1080p, the quality is subjectively indistinguishable from path-traced silicone at thin-feature scales, and the technique has a decade of production deployment. `sim-bevy`'s [shader pipeline (Ch 05)](05-sim-bevy.md) dispatches the SSS pass after the G-buffer is populated and before the final composite; the SSS pass reads per-vertex thickness (interpolated to per-pixel) as a shader attribute from the physics.
...
- [Part 7 Ch 00 (SDF primitive)](../70-sdf-pipeline/00-sdf-primitive.md)'s `MaterialField` struct gains a non-mechanical field: the `DiffusionProfile`. It does not feed the FEM solver; it feeds the renderer. The struct gains a new member without changing how the solver consumes the existing mechanical fields.
...
- [Phase I of build-order](../110-crate/03-build-order.md#the-committed-order) lists Phase I's deliverable as the `sim-bevy` visual layer reaching the ceiling defined in [Part 0 Ch 01 — ceiling](../00-context/01-ceiling.md); SSS is a named requirement within that.
