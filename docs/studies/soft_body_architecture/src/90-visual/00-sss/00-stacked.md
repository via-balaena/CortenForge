...
`MAX_LAYERS` is set to 8 because the shader's per-pixel SSS pass iterates the stack entries at fixed loop bound: the cost is proportional to `MAX_LAYERS`, not to the per-pixel `count`, so raising the cap past the realistic design envelope pays inner-loop cost with no visual payoff. Two to three layers covers the realistic silicone-composite design envelope; the extra headroom is for gradient configurations that discretize a smooth stiffness variation into 4–6 "effective layers" per the [Part 2 Ch 09 spatial-field sampling rule](../../20-materials/09-spatial-fields/01-sampling.md).
...
For gradient-material authoring (a smoothly-varying stiffness field with no explicit layer labels), `cf-design` leaves `layer_id` as `None` and `sim-soft`'s `readout/` crate runs a small k-means-style clustering on the per-tet material vectors at scene-ingest time, partitioning them into 4–6 effective layers per the [Part 2 Ch 09 spatial-field sampling rule](../../20-materials/09-spatial-fields/01-sampling.md). The gradient case is the fallback; the sharp-interface case (explicit `layer_id` from authoring) is the norm.
...
- **Layer identity is an explicit optional field `layer_id` on [`MaterialField`](../../70-sdf-pipeline/00-sdf-primitive.md).** Sharp-interface authoring populates it from `cf-design`; gradient-material authoring leaves it `None` and `readout/` clusters per-tet material vectors into 4–6 effective layers.
...
- **Up to 8 layers supported, ≈2–3 typical.** Cap is GPU texture-array depth on consumer hardware, not a physics limit; realistic silicone-composite designs sit well inside.
- **Multi-layer runs use a sum-of-Gaussians `DiffusionProfile` refit, compound via variance addition.** Single-layer runs keep the more accurate Christensen-Burley sum-of-exponentials form per [§1](01-bssrdf.md); the refit is a scene-ingest step that trades ≲5% tail-accuracy for closed-form layer compounding.
