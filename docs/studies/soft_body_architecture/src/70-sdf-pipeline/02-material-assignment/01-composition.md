...
- **[Adaptive h-refinement](../03-adaptive-refine.md).** Ch 03's stress-gradient criterion $r_e$ typically captures the fast-varying stress at material-interface bands automatically (a stiffness jump produces a stress gradient, which trips $r_e$). The per-tet interface flag is available as diagnostic metadata; Ch 03's published criterion is stress-gradient only, so the flag's role is reporting rather than scoring. Weighting refinement on the flag would be a forward commitment Ch 03 does not currently back.
...
- **The flag is diagnostic metadata, not a refinement trigger.** [Ch 03 adaptive-refine](../03-adaptive-refine.md)'s stress-gradient criterion already picks up interface-induced stress gradients; [Part 11 Ch 04 testing](../../110-crate/04-testing.md) reports the interface-tet count. The Newton loop does not branch on the flag.
