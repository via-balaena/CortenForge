...
|---|---|---|:---:|:---:|:---:|:---:|
| [PolyFEM](02-sota/00-polyfem.md) | IPC + hyperelastic FEM, research-grade correct | Offline; CPU; C++ | ✓ | — | — | — |
| [NVIDIA Warp](02-sota/01-warp.md) | GPU neo-Hookean, full autograd | No IPC; Python-first; NVIDIA-only | partial | — | ✓ | ✓ |
| [Genesis](02-sota/02-genesis.md) | Differentiable, GPU, multi-physics | Python; pre-release; research-grade | partial | partial | partial | partial |
| [IPC Toolkit](02-sota/03-ipc-toolkit.md) | Reference IPC implementation | Contact only; not a full solver; CPU | partial | — | — | — |
...
| [Houdini FEM Solver](02-sota/06-houdini-jelly.md) | Production-quality visual, FEM-based | Offline; proprietary; not a library | partial | ✓ | — | — |
| [FEBio](02-sota/07-febio.md) | Biomechanics hyperelastic + viscoelastic | Offline CPU; biomech scope only | partial | — | — | — |
...
1. **No existing solver covers all four axes simultaneously.** Warp reaches two axes at `✓` (Real-time, Differentiable) but misses Physical (no IPC) and Visual; Genesis shows `partial` across the board in the 2026-04-20 snapshot (Pass 2 may upgrade some); PolyFEM reaches Physical `✓` offline; Flex and Houdini FEM Solver each reach Visual `✓` from opposite sides — Flex via real-time PBD (`—` on Physical and Differentiable), Houdini via offline FEM (`partial` on Physical, `—` on Real-time and Differentiable). The empty cells in the rightmost columns above are the gap [Ch 01 sub-chapter 02](01-ceiling/02-the-gap.md) names. See [Ch 02 §09 cross-compare](02-sota/09-cross-compare.md) for the full pairwise-intersection analysis.
