...
Then jump to [Part 11 — Crate Architecture](../110-crate/00-module-layout.md) for the shape of `sim-soft` at the module and trait level, and [Part 12 — Roadmap](../120-roadmap/00-dependencies.md) for the build order and [what "minimum viable" means](../120-roadmap/02-first-working.md). If you are going to touch the contact layer, [Part 4](../40-contact/00-why-ipc.md) and [Part 5](../50-time-integration/00-backward-euler.md) are required reading; if the GPU layer, [Part 8](../80-gpu/00-wgpu-layout.md).
...
The [thesis](../10-physical/03-thesis.md) is worth reading at some point but is less central for this reader; the ML reader's question is usually *is this stack differentiable in the way I need, and how fast,* and the answer to both is in Parts 6 and 8.
...
[Part 4 — Contact](../40-contact/00-why-ipc.md) is where the book may diverge from a biomech reader's expectation — `sim-soft` commits to IPC rather than the mortar / penalty / augmented-Lagrangian contact formulations common in biomech FEM. The [thesis](../10-physical/03-thesis.md) and [Part 4 Ch 00 — Why IPC beats penalty and impulse](../40-contact/00-why-ipc.md) are where the reasoning lives.
...
Then [Part 10 — Optimization Loop](../100-optimization/00-forward.md) for the design-print-rate loop that soft robotics typically lacks on the simulation side. [Part 11](../110-crate/00-module-layout.md) for the module architecture; [Part 7](../70-sdf-pipeline/00-sdf-primitive.md) for the SDF authoring path that connects [`cf-design`](../110-crate/02-coupling/04-cf-design.md) to `sim-soft`.
