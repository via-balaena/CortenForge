...
The IPC barrier $b(d) = -(d - \hat d)^2 \ln(d/\hat d)$ has a different pathology. The expression is smooth on $(0, \hat d)$, with $b(\hat d) = 0$ at the smooth junction and $b(d) \to +\infty$ as $d \to 0^+$. Naïve chain-rule evaluation through $\ln(d/\hat d)$ is numerically fragile near both endpoints: small-quantity arithmetic at the smooth junction $d = \hat d$ where both factors are near-zero, and unbounded-growth arithmetic near $d \to 0^+$ where the chain rule's intermediate terms lose digits in the barrier's higher derivatives — exactly the quantities the Newton solver needs accurate. The hand-stabilized VJP rearranges the algebra (log1p-style recentering, Taylor expansion at the junction) to keep the forward and the backward in sync with the mathematical form.
