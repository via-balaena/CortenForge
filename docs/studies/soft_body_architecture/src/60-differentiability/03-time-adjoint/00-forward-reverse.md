...
The price is memory: every backward step needs access to the forward step's factored $A_t$ and its residual-Jacobian $\partial r_t/\partial \theta$. Naively, the forward pass stores one such pair per step; [Ch 04 checkpointing](../04-checkpointing.md) ships uniform primal-only checkpointing in Phase D — storing the tiny per-step primal state and re-factoring on backward, at a fixed $\approx 2\times$ forward compute — and evaluates Revolve's $O(\log T)$ schedule as a Phase E candidate for GPU deployments with tight VRAM at long trajectory lengths.
...
- **Forward-trajectory memory is the cost reverse pays; [Ch 04](../04-checkpointing.md) is the mitigation.** A naive O(T) storage of forward factors and residual-Jacobians along the trajectory is unaffordable at long-rollout sizes; Phase D's uniform primal-only checkpointing and Phase E's optional Revolve schedule recover a bounded memory footprint at the cost of re-forward compute during the backward pass.
