...
**GPU sparse solvers for FEM matured.** [NVIDIA Warp](../00-context/02-sota/01-warp.md), [Genesis](../00-context/02-sota/02-genesis.md), and research GPU implementations from Princeton and Stanford demonstrated that preconditioned conjugate gradient on a mesh-derived sparse structure runs at kernel-throughput-bound rates on modern GPUs. Warp's neo-Hookean benchmark on a 4090 clears ~100k tets at 60+ FPS with a single Newton step per frame and warm-started CG; full Newton with two or three inner iterations clears ~30k tets at 60 FPS on the same hardware. This is well inside the per-character budget of a game engine and comfortably inside the per-scene budget of an experience-mode preview for an engineering tool.
