...
This chapter names the traits that form `sim-soft`'s public API surface. Every cross-module interaction routes through one of them; every external crate (`sim-core`, `sim-thermostat`, `sim-bevy`, `sim-ml-chassis`, `cf-design`) sees `sim-soft` as a set of trait implementations, not as a pile of structs.
...
2. **Trait surface is public-stable before implementations are.** The point of pinning the trait surface early is that consumer crates (`sim-core` coupling, `cf-design` bridge, `sim-ml-chassis` reward hookup) can be designed against the trait signatures before `sim-soft`'s internal implementations exist. [Part 11 Ch 03 build order](03-build-order.md) and [Part 12's milestones](../120-roadmap/00-dependencies.md) both rely on this separability — traits ship first, implementations fill in phase-by-phase.
