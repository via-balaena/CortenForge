...
The `assert_reward_breakdown_bit_equal` assertion above is CPU-scoped. GPU atomic-add non-associativity ([Part 8 Ch 03](../../80-gpu/03-gpu-autograd.md)) precludes bit-equal at the GPU backend; §01 cannot run against `GpuTape`. GPU determinism-in-θ is enforced primarily by the [`Differentiable`](../01-traits/00-core.md) and [`ForwardMap`](../../100-optimization/00-forward.md) trait contracts — cross-call state mutation forbidden per the trait docstrings — and secondarily by §03's 5-digit CPU-vs-GPU bar, which catches gross drift outside tolerance but runs one GPU forward only and does not directly exercise GPU cross-call state. A Phase-E GPU-side §01 counterpart (two GPU evaluations at same θ, 5-digit relative tolerance) is a candidate Phase E deliverable; infrastructure-side cross-call state ([`GpuTensorPool`](../../80-gpu/04-chassis-extension/01-gpu-backend.md) reuse, other `GpuDevice`-owned state) is subject to an acquire-use contract specified alongside the chassis GPU extension.
