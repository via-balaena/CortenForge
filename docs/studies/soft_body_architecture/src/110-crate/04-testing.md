...
2. **MuJoCo flex is the regression baseline, not the fidelity target.** The Track 1B platform coverage for MuJoCo flex ([Part 12 Ch 01](../120-roadmap/01-track-1b.md)) exists so `sim-soft` has an already-working soft-body solver to compare against on the same scenes. The comparison is not "sim-soft should match flex numerically" — they solve different PDEs. The comparison is "where the two disagree, the disagreement should be in the direction the [Part 1 Ch 03 thesis](../10-physical/03-thesis.md) predicts" — less volume loss, less popping, more realistic rim deformation. The regression test fails when the disagreement has the wrong sign, not when the numbers differ.
