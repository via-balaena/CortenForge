...
`ForwardMap::evaluate` and `ForwardMap::gradient` take `&mut self` to admit *intra-call* mutability: impl-field scratch, per-call tape-recording hooks, physics-rollout state that lives within a single `evaluate` or `gradient` call. The expected sim-soft impl holds references to the active `Solver`, the scene/mesh, and the [`Observable`](../01-traits/00-core.md) readout; the specific struct layout is a Phase D implementation choice, not a trait-level commitment. Impl-field state that is scratch-across-one-call is expected and load-bearing.

*Cross-call* state — anything readable in a later `evaluate` or `gradient` call and affecting output — is forbidden. Warm-start iterates persisting across $\theta$ evaluations, adaptive reward-scaling state updated from a previous call, accumulated per-call statistics affecting output: all violate the [γ-locked determinism-in-θ contract from Part 10 Ch 00](../../100-optimization/00-forward.md) (elaborated §below) and invalidate the [BayesOpt GP cache from Part 10 Ch 02](../../100-optimization/02-bayesopt.md) that maintains $(\theta_i, R_\text{sim}(\theta_i), \nabla_\theta R_\text{sim}(\theta_i))$ training data across the optimization run. Same mutation-discipline template as [`Differentiable`](../01-traits/00-core.md) and [`Solver`](../01-traits/00-core.md) in §110 Ch 01 core and the chassis [`Preconditioner`](../../80-gpu/02-sparse-solvers/03-preconditioning.md) — uniform across chassis-owned and sim-soft-owned impl-side traits at trait-contract level.
...
The `VjpOp` substrate honoring this contract is chassis-owned, not sim-soft-local. Both CPU `Box<dyn VjpOp>` impls and GPU-registered WGSL VJP kernels satisfy the same determinism-in-θ-within-5-digits discipline per [Part 8 Ch 04 §02 author contract](../../80-gpu/04-chassis-extension/02-vjp-api.md); the chassis trait docstring on `VjpOp` is the canonical authority. Forbidden modes (RNG without passed seed, wall-clock/env reads, mutable global state, HashMap iteration order, cross-call mutation on CPU impls, non-deterministic subgroup ops on GPU kernels) break the contract; book-mandated gradcheck catches high-magnitude violations, the A.4 §4 tolerance band absorbs low-magnitude drift.
