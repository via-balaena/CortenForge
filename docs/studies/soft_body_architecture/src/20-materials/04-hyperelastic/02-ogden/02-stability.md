...
`Ogden` declares `InversionHandling::RequireOrientation` on its [`ValidityDomain`](../../00-trait-hierarchy/02-validity.md), matching neo-Hookean and Mooney-Rivlin. If $\det F \leq 0$ reaches any principal-stretch evaluation — which would leave at least one $\lambda_j$ negative or zero, and $\lambda_j^{\alpha_i}$ for non-integer $\alpha_i$ is then either complex or undefined — the evaluation is undefined there. ⚠ Note how the shipped laws actually behave: `NeoHookean` / `Yeoh` do **not** panic on a non-positive $\det F$ — they return `NaN` via $\ln$ of it — and enforcement is a step-boundary sweep in the Newton solver (`check_validity_at_step_start`), not a contact barrier. An `Ogden` impl should assume the same division of labour.

The volumetric term $U_\text{vol}(J)$ diverges as $J \to 0^+$ in both the compressible and modified forms — the standard $\tfrac{\lambda}{2}(\ln J)^2$ choice goes to $+\infty$ — so the energy itself guarantees an infinite barrier against inversion in the closed-form sense. The constitutive law is not expected to handle the post-inversion regime. The solver's step-boundary validity sweep rejects a non-positive $\det F$ at any Gauss point, on the corner block, or at any reference corner, so an inverted state cannot be *accepted* as a step's start or result — though Newton's intermediate trial states are not gated, and the line search rejects those by their residual. A contact barrier only covers inversions of contact origin, which is a strict subset (a prescribed-displacement warm start can invert an element with no contact anywhere in the scene).
...
- **Inversion is not the constitutive law's job.** `InversionHandling::RequireOrientation` declares the domain; the solver's step-boundary validity sweep is what rejects a configuration outside it. (The shipped laws return `NaN` rather than panicking there — see the neo-Hookean impl leaf.)
