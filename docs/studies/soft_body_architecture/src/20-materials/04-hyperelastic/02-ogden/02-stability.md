...
The volumetric term $U_\text{vol}(J)$ diverges as $J \to 0^+$ in both the compressible and modified forms — the standard $\tfrac{\lambda}{2}(\ln J)^2$ choice goes to $+\infty$ — so the energy itself guarantees an infinite barrier against inversion in the closed-form sense. The constitutive law is not expected to handle the post-inversion regime; keeping the iterates out of it is the solver's job — the step-boundary validity sweep rejects a non-positive $\det F$ at any Gauss point. A contact barrier only covers inversions of contact origin, which is a strict subset (a prescribed-displacement warm start can invert an element with no contact anywhere in the scene).
...
- **Inversion is not the constitutive law's job.** `InversionHandling::RequireOrientation` declares the domain; the solver's step-boundary validity sweep is what actually keeps the configuration from arising. (The shipped laws return `NaN` rather than panicking there — see the neo-Hookean impl leaf.)
