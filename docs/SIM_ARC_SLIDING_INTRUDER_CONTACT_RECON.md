...
6. **Misconfiguration of `interior_cutoff < d_hat`** would give an empty active band `[max(-c, -d_hat), d_hat] = [-c, d_hat]` that excludes legitimate near-contact pairs. cf-device-design's `2 × cavity_inset_m = 6 mm` is comfortably `>> d̂ = 1 mm` so no current consumer hits this. Mitigation (as-built CR.1): `assert!`-guards on positivity + finiteness + `> d_hat` in `with_params_and_interior_cutoff` (release-mode panic). The spec originally called for `debug_assert!` (cheaper, debug-only); CR.5 cold-read flagged the spec/code divergence and the resolution was to keep `assert!` because (a) sim-soft is a library crate with multiple consumers, (b) the misconfiguration is silent-wrong-results (not just slow), and (c) the release-mode cost is trivial (3 scalar comparisons at constructor time, called once per ramp setup). The 3 `#[should_panic]` precondition tests in `sim/L0/soft/tests/penalty_interior_cutoff.rs` lock the contract.
...
Tool launched with `cargo run --release -p cf-device-design -- ~/scans/sock_over_capsule.cleaned.stl`. Sliding-mode Simulate ran to completion in **~3-4 minutes wall-clock** (user-perceived: from clicking Simulate to first scrub-step screenshot, accounting for the post-run idle); precise instrumentation deferred — the stderr writer flushed only the first step's Newton-fallback lines before going quiet, so per-step wall-clock isn't extractable from the log. Headline UI panel readout:
