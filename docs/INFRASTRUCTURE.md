...
**Checks**:
- **Scan/mesh guard** — refuses 23 staged mesh, point-cloud and medical
  extensions (see 1.3.1 below for the list and the policy). Runs first: it is a
  safety rule, not a quality one.
...
anatomical scans of a real person — or will be.** Today's fixtures are physical
STAND-INS (a clay form); the rule holds regardless, because the real scans are
the point of the capstone and one miss is unrecoverable. They must never be
committed here.
...
has ever tracked a mesh or a scan. Re-verified 2026-08-31 across **all 23**
guarded extensions — zero tracked, and zero ever added in history:

```sh
EXTS=$(grep -o "icase)\*\.[a-z0-9]*" xtask/hooks/pre-commit \
       | sed 's/icase)//' | sed "s/^/':(icase)/;s/\$/'/" | tr '\n' ' ')
eval "git ls-files -- $EXTS"                                        # index
eval "git log --all --diff-filter=A --name-only --pretty=format: -- $EXTS"
```

⚠ The command builds its pathspecs FROM the hook rather than repeating them, so
it cannot check a stale list — the mistake the previous version of this note
made, having been written when the guard covered seven.
...
|---|---|---|
| `.gitignore` blanket (23 extensions, same set as layer 2) | `git add`, `git add .` | `git add -f`; and an UPPERCASE `PART.STL` wherever `core.ignorecase=false` (Linux, CI) — these patterns are case-sensitive, unlike layer 2's |
...
⚠ **Both layers are ALLOWLISTS, and that is still the largest hole** — but the
policy behind the list CHANGED on 2026-08-31, when the pipeline was first pointed
at a full-resolution body-shaped scan and its sensitivity was raised. That fixture
turned out to be a physical stand-in — which is the right time to widen a guard,
not after the real data lands.

**Old policy: the guard tracked the formats `mesh-io` reads and writes**, and so
it lagged them. `.step`/`.stp` were in neither layer until 2026-08-27 despite
`MeshFormat::Step` existing all along. Under that policy a scan exported as
`.glb`, `.gltf`, `.off`, `.pcd`, `.e57` or `.fbx` passed both layers untouched.

**New policy: the guard tracks what a SCANNER OR MEDICAL SOURCE CAN EMIT**, which
is a superset of what we can read. It now covers 23 extensions — the 7 mesh
formats plus point clouds (`.e57` `.pcd` `.pts` `.ptx` `.las` `.laz`), medical
(`.dcm` `.nii`) and interchange (`.glb` `.gltf` `.fbx` `.dae` `.off` `.amf`
`.3ds` `.wrl`). Six of the seven formats the old note named as walking straight
through are now blocked.

⇒ **The rule that replaces "when `mesh-io` learns a format":** both layers and
their tests must learn a format when **any device we might point at a body can
export it**, whether or not this codebase can open it. The tests derive their
list from the hook's own pathspec (`guarded_extensions()` in
`xtask/src/setup.rs`), so extending the guard extends its coverage automatically.

⚠ **`.xyz` remains deliberately uncovered.** It is a point-cloud extension, but
it is also the default for ordinary numeric data, and `.gitignore` would swallow
those silently. A guard that trains people to reach for `git add -f` is worse
than the gap it leaves. **This is the known, accepted hole** — plus, as always,
any extension nobody has thought of.
