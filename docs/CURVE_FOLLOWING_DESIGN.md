...
- **Registration-pin disconnection on multi-surface cup-piece hosts** (recon-3 §R3-2 2026-05-22). Post-S5 the iter-1 STL set's §R1 connectivity-invariant inspector surfaced 5–6 connected components per cup piece — the registration pin cylinders survived as separate ~168–188-face shells inside the cup-wall volume rather than absorbing into the surrounding mesh. Recon-3 §R3-3's synthetic shell-host probe (`mesh_csg::tests::apply_mating_transforms_absorbs_contained_cylinder_into_shell_host` + `_merges_protruding_cylinder_into_shell_host`) showed manifold3d's boolean union behaves correctly on synthetic geometry; the production failure lives at a deeper layer of the cf-cast pipeline (likely the multi-op + complex curved-body interaction with MC-quantized surfaces). **Mitigation (recon-3 §R3-2 (α) pick):** each registration pin is now **anchored to the bounding outer surface** — `pin_offset = bounding_dist - pin_half_length_m + PIN_PROTRUSION_M` (default `PIN_PROTRUSION_M = 0.5 mm`) — so the pin's lateral cylindrical surface protrudes through the cup-wall outer face at a circular cross-section. This forces manifold3d onto its most-robust surface-vs-surface intersection path, sidestepping the unknown pipeline-layer disconnection cause entirely. Workshop trade-off: each pin's outer face shows a ≈ 3 mm Ø × 0.5 mm circular bump (4 per cup piece, below FDM bead-width × 2 the platform pocket already absorbs) and the pin's `-axis` tip sits 0.5 mm INTO the body cavity in the iter-1 5 mm wall (≈ 3 mm Ø × 0.5 mm silicone dimple on the cast, well within recon-3 §R3-2's ≤ 2 mm silicone-demold tolerance; default `pin_half_length_m = 3 mm`). The `tools/cf-cast-cli::CastConfig::validate` cross-field gate enforces `2 × pin_half_length_m ≤ wall_thickness_m + PIN_PROTRUSION_M + MAX_REGISTRATION_DIMPLE_M` (2 mm dimple tolerance — recon-3 §R3-2's "silicone tolerates ≤ 2 mm undercuts on demold"). The §R1 workshop-fixture inspector at `design/cf-cast/tests/iter_connectivity_inspector.rs` (`INSPECT_STL_R1=1` mode) remains the load-bearing falsification gate; S5's bit-precise pin/socket fit invariant is preserved (`pin_transforms_positive_socket_params_inflate_per_spec` covers it). See `docs/CF_CAST_REGISTRATION_PIN_DISCONNECTION_RECON_PLAN_V2.md` §R3-2 and `MEMORY.md`'s `project-cf-cast-registration-pin-disconnection-recon3` entry.
