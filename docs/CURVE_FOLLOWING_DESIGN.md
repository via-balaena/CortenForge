...
- **CSG numerical issues at the ribbon ∩ mold_cup intersection** — SDF intersection is min(a, b), which is correct geometrically but produces marching-cubes artifacts when the surfaces are coincident or near-tangent. Mitigation: add a small bias (~0.5 cell-size) to the ribbon SDF so the intersection is clean. v1 already uses this trick for the clip-body overlap (`CLIP_BODY_OVERLAP_M = 0.5 mm`). **Update (2026-05-23, recon-4 (P)):** the seam cut is the SDF half-space intersect described above (the recon-4 architectural correction reverted S4's `MatingTransform::SeamTrim` migration); MC's linear-SDF interpolation property places seam-cap vertices on the seam plane to f64 precision (`mesh_csg::tests::f4_synthetic_presdf_seam_is_bit_precise_flat` + `piece::tests::mating_face_is_mathematically_flat_and_coplanar` lock the gate). See `docs/CF_CAST_SEAM_FACE_FILM_RECON_PLAN.md` §F-2/§F-4 and the §"Per-piece SDF construction" note above.

- **SDF↔mesh-CSG paradigm boundary placement** (recon-4 §F-2 2026-05-23). cf-cast mixes two geometric paradigms: SDF/MC (continuous, analytical) for bulk geometry (body + bounding + seam half-space — all curve-following, continuous deformations) and mesh-CSG (discrete, manifold3d on MC-quantized meshes) for fine features (exact cylinder primitives with bit-precise diametral fit — registration pins, T-bar/T-slot, plug-shaft, pour-gate, funnel-nipple). Each feature belongs to the paradigm it geometrically fits. S4 of the mating-features arc (2026-05-22) put the seam in the wrong column — moved it from SDF (where it geometrically belongs as part of bulk geometry) to mesh-CSG. The post-MC `trim_by_plane` produced a thin film of cup-wall material capping the body-cavity opening on each cup piece's seam face under the production curved-body × MC × trim interaction (synthetic axis-aligned probes ruled out manifold3d + MC quantization in isolation; the exact cellular-MC mechanism remains uncharacterised because the recon-4 (P) pick sidesteps it). **Mitigation (recon-4 (P) pick):** revert S4's seam migration — the seam returns to the SDF half-space intersect (`bounding ∖ body ∩ halfspace(side)` per `Ribbon::halfspace_solid`); S5/S6/S7 mating-features cylinders remain mesh-CSG post-MC; the recon-3 (α) split-normal-axis protrusion pin design (a symptomatic mitigation for a related pin-disconnection signal that was downstream of the same S4 architecture) reverts to the workshop-ergonomic pre-recon-2 binormal-axis annulus-midpoint pin (per `docs/CF_CAST_SEAM_FACE_FILM_RECON_PLAN.md` §F-3 confirming `mesh_csg::tests::f3_synthetic_presdf_seam_plus_sdf_pin_is_one_component` + the recon-3 §R3-3 mesh-CSG-union shell-host characterisation). The film bug is architecturally impossible under (P) (the body-cavity opening is established by SDF arithmetic, no post-MC cap operation runs); flatness is preserved by MC's linear-SDF interpolation (§F-4); the workshop-visible binormal-axis pin ridge protrudes past each Negative cup piece's seam face into the matching Positive socket. The §R1 workshop-fixture inspector at `design/cf-cast/tests/iter_connectivity_inspector.rs` (`INSPECT_STL_R1=1` mode) is retained as a regression guard against any future seam migration. See `docs/CF_CAST_SEAM_FACE_FILM_RECON_PLAN.md` §F-2/§F-3/§F-4/§F-5 and `MEMORY.md`'s `project-cf-cast-seam-face-film-recon4` entry.
