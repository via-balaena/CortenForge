...
**Status:** Commits 1 (scaffolding `02bceb9c`) + 2 (PLY load + `ViewerInput` `093f0410`) + 3 (geometry rendering `79b97339`) + commit-3 cold-read fixes (`b46d0b99`) + 4 (colormap pipeline `0c3ab996`) + commit-4 visual-review corrections (`8b53f296` — TwoSlopeNorm + unlit + saturated bwr) + commit-4 cold-read doc fixes (`7ecbb54b`) + 5 (orbit camera + UI `93abde78`) + sphere-sdf-eval gradient_magnitude interlude (`8d9991c5`) + commit-5 cold-read fixes (`0245ef82`) + N+1 cold-read fixes (`7b6a4b1d`) + 6 (CLI flags `cb69ee56`) + CI tests-debug propagation (`a81cdcf5`, sibling) + 7 (sphere-sdf-eval README MeshLab → cf-view migration `c47231a5`) + N+1 frontmatter fix (`9fe022ed` — drop stale "external" qualifier) + design-doc refresh + iter 1.10 (`ec6e2e46`) + **print_summary cf-view migration (`6f1b4edb`, sibling) + N+1 "scalar" dropdown precision (`d9f81eea`, sibling)** shipped on `dev`. Plan iter-1 locked + iter-1.3/1.4/1.5/1.6/1.7/1.8/1.9 + iter-1.10 + iter-1.11 (sibling commits 6f1b4edb + d9f81eea closing iter 1.10's call (D) print_summary half-migration thread) all banked. **Commits 5 + 6 + 7 all visually reviewed + passed first try** — third consecutive first-try success on a non-pipeline commit, calibration in `feedback_visual_review_is_the_test` confirmed 3-for-3 across input/lifecycle (commit 5), CLI/axis (commit 6), and pure-documentation (commit 7) classes. **Iter-1 still-open #8 (CI tests-debug propagation) CLOSED 2026-05-04 via sibling commit `a81cdcf5`.** 56/56 cf-viewer unit tests (was 36 pre-commit-6 — +20 new across cli + mesh + camera + lib). Multi-scalar verification gap closed via the sphere-sdf-eval interlude (PLY now carries 2 scalars; dropdown switching visually verified). Branch strategy revised mid-commit-1 to single-branch flow per `feedback_single_active_branch`. **Sphere-sdf-eval consumer migration story FULLY CLOSED 2026-05-04** — README + frontmatter + print_summary all migrated; the workspace's first sim-soft consumer is end-to-end on cf-view with no legacy-viewer refs in any surface. **Next step: optional commit 8 (cf-viewer crate README) OR resume sim-soft examples arc PR1 at row 2 (`hollow-shell-sdf`) — user-call which path comes next.**
...
8. ~~**CI `tests-debug` propagation gap**~~ — RESOLVED at sibling commit `a81cdcf5` (2026-05-04). cf-viewer is now in `.github/workflows/quality-gate.yml` `tests-debug` (`-p cf-viewer` appended to the existing crate list, plus a 3-line inclusion-rationale comment noting the tests are pure-function and headless-CI safe). Surfaced commit-2 cold-read; deferred through commits 3 + 4 + 5; landed alongside commit 6 as a cleanly-scoped sibling rather than bundled into the feat commit. Item still listed here for cross-doc audit-trail consistency with the arc memo's #5.
...
- **2026-05-04 (iter 1.9 — commit-6 head-architect calls + sibling-CI ship + visually reviewed first try):** Three architectural calls made at commit-6 authoring time per `feedback_master_architect_delegation`:

  **(A) clap derive direct dep** — `clap = { version = "4", features = ["derive"] }` added to `cf-viewer/Cargo.toml`. Workspace has no clap pin today (xtask carries the same direct dep at version 4 with derive); cf-viewer is the second consumer. Considered workspace-pinning instead, but the second-consumer threshold is conventional for promoting to a workspace dep — defer until a third consumer arrives. Derive macros + `ValueEnum` + `#[value(name = "+X")]` for the unconventional `+X|+Y|+Z` wire format; default values via the `default_value = "+Z"` string form (parsed through `ValueEnum::from_str`) rather than `default_value_t = ...` to sidestep the `Display` requirement.

  **(B) Seeded resources via `insert_resource`** — replaced `init_resource::<Selection>()` with `insert_resource(seed_selection(&cli, &input.scalar_names)?)` so the CLI's `--scalar=<name>` is validated against the loaded PLY's scalar list before App::new() (early bail with `anyhow::bail!` listing available names). UpAxis follows the same pattern: `insert_resource(cli.up.into())`. Both seeded resources are visible to systems on the very first frame — the dropdowns then act as runtime overrides on top of the seed (e.g., `--scalar=signed_distance` opens with that scalar pre-selected; user can still switch via the dropdown).

  **(C) UpAxis Resource at lib root + 3-way swap matrix** — moved iter-2 still-open #2's hardcoded `[x, z, y]` to a parameterized `UpAxis` enum with three variants. Empirical verification before locking the matrix: input-X→Bevy-Y means `(x,y,z)→(y,x,z)` (X↔Y reflection, det=-1, winding flips); input-Y→Bevy-Y means identity (det=+1, no flip); input-Z→Bevy-Y means `(x,y,z)→(x,z,y)` (Y↔Z reflection, det=-1, winding flips). `UpAxis::flips_winding()` is the single source of truth for the parity-flip predicate (`!matches!(self, PlusY)`). Threading: pure helpers (`vertex_position`, `vertex_normal`, `build_face_mesh`, `OrbitCamera::framing_for_aabb`) take `up: UpAxis` arg by value (Copy); systems read `Res<UpAxis>` at the boundary and pass through. Idiomatic Bevy + keeps helpers testable. `+Z` reproduces today's behavior bit-for-bit (verified by re-running existing visual-review scenarios).

  Recon batch before authoring: `xtask/Cargo.toml` + `xtask/src/main.rs` for clap precedent; `grep "^clap" Cargo.toml` (empty — no workspace pin); the three swap call-sites in `mesh.rs::vertex_position` + `mesh.rs::build_face_mesh` (winding flip) + `camera.rs::framing_for_aabb`; `quality-gate.yml` `tests-debug` crate-list shape (line 170-177). All five recon hits informed the implementation; no surprises during authoring.

  **Sibling commit `a81cdcf5`** — closed iter-1 still-open #8 (CI `tests-debug` propagation) by appending `-p cf-viewer` to the existing test list with a 3-line inclusion-rationale comment. Cleanly-scoped sibling rather than bundled into the feat commit so the diff intent is unambiguous. Item was OVERDUE since commit-2 cold-read; deferred through commits 3 + 4 + 5; resolved alongside commit 6 at the sibling's ship time.

  **Visually reviewed + passed first try 2026-05-04** — all 6 verification scenarios passed (default load, `--scalar=signed_distance` override, `--colormap=sequential` override, `--scalar=does_not_exist` errors with available scalars listed, `--up=+Y` renders without the Z↔Y swap, `--help` is readable). **Second consecutive first-try success on an input/lifecycle commit** (commits 5 + 6 both passed first-try; commit 4 needed three iter-cycles). This strengthens the `feedback_visual_review_is_the_test` calibration: the math-vs-render gap is specifically a viz-pipeline concern (lighting × tonemap × colormap × normalization × material flags); commits that don't touch that chain reliably pass first-try. The diagnostic question stands: *does this code touch how the data's color reaches the screen, or just what gets rendered and when?*

  **`cargo fmt --all` gotcha bit a fourth time** (commits 3 + 4 + 5 + 6 — four-for-four). Per-crate `cargo fmt -p cf-viewer -- --check` reported clean while `--all` flagged 3 reflows in commit 6: `bail!()` line-collapse in `cli.rs:129`, multi-line `assert_eq!` reflow in cli tests at 207, and an `assert!` macro-arg reflow at 261. Pre-commit hook caught them; iterated to clean. Calibrated rule from `project_git_and_hook_gotchas.md` continues to hold: **always run `cargo fmt --all` before commit**, never trust per-crate.

  **Tests:** 56/56 unit tests passing (was 36 pre-commit-6). +20 new tests: 12 in `cli.rs` (Cli parse defaults + each flag's wire format + each enum's From conversion + seed_selection success/error/empty paths), 4 in `mesh.rs` (vertex_position + build_face_mesh under each of PlusX / PlusY / PlusZ, with the existing tests renamed to be axis-explicit), 2 in `camera.rs` (framing_for_aabb under PlusY identity + PlusX swap), 2 in `lib.rs` (UpAxis::default + flips_winding truth-table). Existing tests updated to pass `UpAxis::PlusZ` explicitly to reproduce today's behavior.

  **Cold-read finding (caught pre-commit):** `camera.rs` module doc claimed "`mesh.rs` swaps Z↔Y on load so the input's Z-up frame already matches Bevy's by the time the camera sees it" — was PlusZ-specific after the parameterization. Updated to reference the parameterized swap explicitly (`mesh::vertex_position` / `OrbitCamera::framing_for_aabb`).

  **Bevy 0.18 design pattern banked (commit 6):** `clap::ValueEnum` derive plays cleanly with non-default Bevy resource types when the wire format is exotic (`+X|+Y|+Z`). Use `#[value(name = "+X")]` for the canonical name; `#[derive(Default)]` + `#[default]` on the variant for the variant-as-default pattern (clippy::derivable_impls flags the manual `impl Default for E` form — clippy bit at first compile and was fixed before commit). For seed-from-CLI Bevy resources, `insert_resource` (not `init_resource`) is the right tool because the seeded value depends on data that's only available at runtime (`ViewerInput::scalar_names`).

  **Next iteration entry point:** commit 7 — `examples/sim-soft/sphere-sdf-eval/README.md` "Visuals" section retrofit from the MeshLab two-filter chain to `cf-view <path>`. End-to-end smoke test of the entire arc against the same fixture that motivated the arc's creation. After commit 7 ships + is reviewed, sim-soft examples arc PR1 resumes at row 2 (`hollow-shell-sdf`), authored against the now-stable cf-viewer; mesh-v1.0 retrofit (Q7) is its own followup PR after PR1 finishes.

- **2026-05-04 (iter 1.10 — commit-7 sphere-sdf-eval consumer migration + N+1 frontmatter coherence fix):** Commit 7 (`c47231a5`) shipped the smallest commit in the arc — README-only, no Rust touched. Visuals section's MeshLab two-filter chain (`Filters → Quality Measure and Computations → Per Vertex Quality Function`, `func q = signed_distance`, `Filters → Color Creation and Processing → Colorize by Vertex Quality`) plus the ParaView Coloring-panel aside both deleted; replaced by a single `cargo run -p cf-viewer -- <path>` invocation + auto-discovery + dropdown explanation + `--scalar=signed_distance` scripted-reproducibility alternative. 16 insertions, 11 deletions, net +5 lines.

  Four head-architect calls per `feedback_master_architect_delegation`:

  **(A) Replace text content + scope:** delete the entire MeshLab+ParaView block (preamble + filter chain + ParaView aside); replace with cf-view invocation + tight runtime-dropdown explanation + scripted-CLI alternative. Preserve both "what you should see" bulleted lists verbatim — they're colormap-agnostic enough to read correctly under cf-view's bwr/viridis defaults.

  **(B) `--scalar=<name>` mention placement:** bare `cargo run -p cf-viewer -- <path>` is the primary path; alphabetical-first-pick lands on `gradient_magnitude` (sequential viridis); runtime side-panel scalar dropdown switches to `signed_distance` (divergent bwr — blue interior, white surface band, red exterior). The `--scalar=signed_distance` CLI flag shown immediately after as "for scripted reproducibility" — frames the dropdown as the discoverable runtime affordance and the CLI flag as the reproducibility hook.

  **(C) Screenshots:** none. Per `feedback_code_speaks` + the cf-viewer arc not yet closed (UI drifts through Q7 retrofit), screenshots couple the README to a particular UI version.

  **(D) Other refs in the README:** README grep clean; only the Visuals block carried legacy refs. Cross-section coherence flag surfaced via broader recon: `examples/sim-soft/sphere-sdf-eval/src/main.rs:514` carries a `print_summary` `println!` that says `"open in MeshLab / ParaView and colormap by either scalar"` — a half-migration if left. **RESOLVED 2026-05-04 via sibling commits `6f1b4edb` + `d9f81eea`** — see iter 1.11. User picked Option α-immediate (mirrors `a81cdcf5` precedent for cleanly-scoped sibling cleanups).

  **Visually reviewed + passed first try 2026-05-04** — third consecutive first-try success. Commits 5 (input/lifecycle) + 6 (CLI/axis) + 7 (pure documentation) all passed first-try. The risk-class calibration in `feedback_visual_review_is_the_test` now spans three classes; the iter-loop expectation is reserved for pipeline-touching commits (commit 4's three-iter cycle was math-vs-render, specifically lighting × tonemap × colormap × normalization × material flags). For commit 7 specifically the visual review surface was the rendered README + running its instructions verbatim against the real fixture (PLY at `out/sdf_grid.ply`, 26911 bytes); cf-view's behaviour matched the README's claims — `gradient_magnitude` selected on launch, dropdown switches to `signed_distance` cleanly, `--scalar=signed_distance` opens with that scalar pre-selected.

  **N+1 cold-read pass (`9fe022ed`) caught one cross-section coherence drift:** the README frontmatter's closing phrase `"for external colormap rendering"` read unambiguously pre-cf-view (when MeshLab / ParaView truly were external tools), but post-migration the recommended viewer is in-workspace. A fresh reader could trip on "external" as "outside the workspace" while the next-paragraph-down points at `cargo run -p cf-viewer`. One-word tightening: drop "external" → `"for colormap rendering"`. Classic N+1 catch — the diff itself was internally consistent; **adjacent prose shifted in meaning when the workspace's tool surface moved.** Bank-worthy as a doc-coherence-after-tool-migration pattern: when a workspace adds a new in-tree tool that supersedes external ones, sweep adjacent docs for "external" / "third-party" / tool-specific qualifiers that may now read oddly. Useful for the Q7 mesh-v1.0 retrofit PR (9 README updates, each a candidate site for the same drift).

  **Recon batch before authoring:** `grep -rE "meshlab|MeshLab|f3d|func q =|Quality Measure|Colorize by Vertex Quality" examples/sim-soft/sphere-sdf-eval/` — 5 hits (4 in README Visuals, 1 in `src/main.rs:514`); `grep -nE "^fn verify_|^pub fn verify_" examples/sim-soft/sphere-sdf-eval/src/main.rs` — 9 verify_* anchors confirmed matching README §Numerical-anchors enumeration; `ls -la examples/sim-soft/sphere-sdf-eval/out/` — `sdf_grid.ply` (26911 bytes, May 4 2026) confirmed on disk; `cf-viewer/src/cli.rs` end-to-end — `cargo run -p cf-viewer -- <path>` positional arg + `--scalar=<name>` + `--colormap=<auto|...>` + `--up=<+X|+Y|+Z>` confirmed matching the README's invocation forms.

  **Sphere-sdf-eval consumer milestone REACHED:** the cf-viewer arc is now end-to-end working against ONE real consumer (sphere-sdf-eval). PR-shape commit segmentation is 7 of 7-8 done (commit 8 cf-viewer/README.md remains optional). Two follow-on paths, user-call which comes next:

  - **(α) Optional commit 8 — `cf-viewer/README.md`:** documents the binary's CLI surface + supported PLY shapes (per the locked PR-shape's optional commit). Small file, mostly cross-references + a usage block. Not blocking sim-soft arc resumption.
  - **(β) Resume sim-soft examples arc PR1 at row 2 (`hollow-shell-sdf`)** — see `sim/L0/soft/EXAMPLE_INVENTORY.md`. The viewer arc has been authored against ONE real consumer (sphere-sdf-eval); row 2 is the second consumer that surfaces refinements for cf-viewer. Per the PR-shape lock, the Q7 mesh-v1.0 retrofit PR comes after sim-soft PR1 finishes (also picks up iter-1 still-open #5 crease-angle splitting polish).

  **Cargo fmt --all gotcha did NOT bite at commits 7 + 9fe022ed** (no Rust touched in either) — first commits in the arc to dodge the four-for-four streak. Pre-commit hook reported `→ No Rust/Cargo files staged — skipping clippy.` cleanly on both; fmt passed. The fmt-all gotcha calibration stands for any future Rust-touching commit.

  **Next iteration entry point:** see iter 1.11 — sibling-commit-candidate from call (D) shipped same-day.

- **2026-05-04 (iter 1.11 — sibling commits `6f1b4edb` + `d9f81eea`: print_summary cf-view migration + N+1 "scalar" dropdown precision):** Closes the half-migration thread iter 1.10's call (D) deferred. README pointed at cf-view but `cargo run`'s stdout still mentioned MeshLab / ParaView — sphere-sdf-eval was the workspace's first sim-soft consumer of cf-view; leaving the half-migration in place would have surfaced the inconsistency every time the example ran.

  **Sibling commit `6f1b4edb` — print_summary cf-view migration.** Replaces the 4-line legacy-viewer block in `print_summary` (lines 514-517) with cf-view guidance mirroring the README's prose tone: cf-view header (`open in cf-view, the workspace's unified visual-review viewer:`) + bare invocation (`cargo run -p cf-viewer -- <path>`) + per-scalar launch/dropdown pedagogy (default-picks `gradient_magnitude` (Eikonal diagnostic; dip near origin where central diffs smooth the kink) → dropdown switches to `signed_distance` (radial SDF, divergent bwr — blue interior → red exterior)). 6 insertions / 4 deletions; +2 net lines vs the original 4-line block (one for the cargo invocation header, one for the per-scalar pedagogy split). Eikonal-diagnostic + radial-SDF pedagogy preserved verbatim-adjacent.

  **Four head-architect calls per `feedback_master_architect_delegation`:** (A) replace the entire MeshLab/ParaView block (4 lines) with cf-view-specific guidance — staying within +/- 1 of the original 4-line budget; (B) match existing `println!("...");` single-line format strings + 9-space prose indent matching surrounding label-aligned text + 11-space invocation indent matching `extras["..."]` lines above; (C) generic `<path>` placeholder for the cargo run line — the prior `PLY    : {path.display()}` line one row above already shows the concrete path, so duplicating an absolute filesystem path inside a copy-pasteable command would look noisy; (D) keep per-scalar pedagogy compressed but intact — runtime user audience may not have the README open.

  **Sibling commit `d9f81eea` — N+1 "scalar" dropdown precision.** N+1 cold-read on `6f1b4edb` (per `feedback_thorough_review_before_commit`'s standard discipline + the user-initiated extra pass) caught a slight ambiguity: cf-view's UI has BOTH a scalar dropdown AND a colormap dropdown; the print_summary's `"dropdown switches to signed_distance"` relied on inference (signed_distance is a scalar name → must be the scalar dropdown). One-word insertion (line 517): `dropdown switches` → `scalar dropdown switches`. Mirrors the README's "side-panel scalar dropdown" precision so the print_summary stands on its own without context. 1 insertion / 1 deletion.

  **`cargo fmt --all` gotcha did NOT bite at either sibling commit** — first Rust-touching commits in the arc to dodge the four-for-four streak. Calibration: small, single-indent-level changes (≤6 println lines on one indent depth + a one-word insert) don't trip the multi-line struct expression / macro-arg reflow patterns that bit commits 3+4+5+6. Streak now 4-of-6 Rust-touching commits in the arc; the rule "always run `cargo fmt --all` before commit, never trust per-crate" still stands as the safe default.

  **Sphere-sdf-eval consumer migration story FULLY CLOSED 2026-05-04** — README + frontmatter + print_summary all migrated; the workspace's first sim-soft consumer is end-to-end on cf-view with no legacy-viewer refs in any surface (post-edit `grep -nE "meshlab|MeshLab|paraview|ParaView" examples/sim-soft/sphere-sdf-eval/` exits clean). PR-shape commit segmentation 7 of 7-8 done; commit 8 (cf-viewer/README.md) remains optional.

  **Next iteration entry point:** user-call between (α) optional commit 8 (cf-viewer/README.md) and (β) resume sim-soft examples arc PR1 row 2 (`hollow-shell-sdf`). Sibling-commit candidate from iter 1.10 is now resolved (no longer open). Per the PR-shape lock, the Q7 mesh-v1.0 retrofit PR comes after sim-soft PR1 finishes.
...
**Commits 1 (scaffolding `02bceb9c`) + 2 (PLY load + `ViewerInput` `093f0410`) + 3 (geometry rendering `79b97339`) + commit-3 cold-read fixes (`b46d0b99`) + 4 (colormap pipeline `0c3ab996`) + commit-4 visual-review corrections (`8b53f296`) + commit-4 cold-read doc fixes (`7ecbb54b`) + 5 (orbit camera + UI `93abde78`) + sphere-sdf-eval gradient_magnitude interlude (`8d9991c5`) + commit-5 cold-read fixes (`0245ef82`) + N+1 cold-read fixes (`7b6a4b1d`) + 6 (CLI flags `cb69ee56`) + sibling-CI tests-debug propagation (`a81cdcf5`) + 7 (sphere-sdf-eval README MeshLab → cf-view migration `c47231a5`) + N+1 frontmatter fix (`9fe022ed`) + design-doc refresh + iter 1.10 (`ec6e2e46`) + sibling print_summary cf-view migration (`6f1b4edb`) + N+1 "scalar" dropdown precision (`d9f81eea`) shipped on `dev`. Branch strategy revised mid-commit-1 to single-branch flow per `feedback_single_active_branch`. Commits 5 + 6 + 7 all visually reviewed + passed first try 2026-05-04 — third consecutive first-try success spanning input/lifecycle (5) + CLI/axis (6) + pure-documentation (7) classes. Iter-1 still-open #8 closed. PR-shape commit segmentation 7 of 7-8 done. Sphere-sdf-eval consumer migration story FULLY CLOSED 2026-05-04.**

1. Verify current state: on `dev`, the most recent commit is `d9f81eea` (N+1 "scalar" dropdown precision); the most recent code-changing viewer commit is still `cb69ee56` (commit 6). Working tree should be clean. `cargo test -p cf-viewer` should report 56/56 passing.
2. **User-call: which path next?**
   - **(α) Optional commit 8 — `cf-viewer/README.md`** documenting the binary's CLI surface + supported PLY shapes. Per PR-shape's optional commit. Small file, mostly cross-references + usage block. Closes the cf-viewer arc cleanly at 8 of 7-8.
   - **(β) Resume sim-soft examples arc PR1 at row 2 (`hollow-shell-sdf`)** — see `sim/L0/soft/EXAMPLE_INVENTORY.md`. The viewer arc has been authored against ONE real consumer (sphere-sdf-eval); row 2 is the second consumer that surfaces refinements for cf-viewer.
3. The Q7 mesh-v1.0 retrofit is its own followup PR after sim-soft PR1 finishes; that PR also picks up iter-1 still-open #5 (crease-angle splitting polish commit) before the 9 mesh README updates.
...
- Commit 6: `cargo fmt --all` gotcha bit a **fourth** time — per-crate fmt clean, `--all` flagged 3 reflows in `cli.rs` (one `bail!()` line-collapse + two macro-arg reflows in tests). Four-for-four across commits 3 + 4 + 5 + 6. Pre-commit hook caught them; iterated. **Always run `cargo fmt --all`, never trust per-crate.** Banked at `project_git_and_hook_gotchas.md`.
- Commit 6: `clap = "4"` direct dep (matches xtask precedent — no workspace pin exists today, cf-viewer is the second consumer). For exotic wire formats (`+X|+Y|+Z`), use `#[derive(ValueEnum)]` + `#[value(name = "+X")]` per variant; default values via `default_value = "+Z"` (string parsed through `ValueEnum::from_str`) sidestep the `default_value_t = ...` `Display` requirement. The `#[derive(Default)]` + `#[default]` variant-as-default pattern is the clippy-preferred form (clippy::derivable_impls flags the manual `impl Default for E` form on enums where one variant takes no args). For seed-from-CLI Bevy resources, `insert_resource(seeded)` (not `init_resource::<T>()`) is correct because the seeded value depends on data only available at runtime (`ViewerInput::scalar_names`); validate the CLI's name argument against the loaded data before App::new() and `anyhow::bail!` on miss for a clean fail-fast path.
