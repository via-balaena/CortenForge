...
**Status:** Commits 1 (scaffolding `02bceb9c`) + 2 (PLY load + `ViewerInput` `093f0410`) + 3 (geometry rendering `79b97339`) + commit-3 cold-read fixes (`b46d0b99`) + 4 (colormap pipeline `0c3ab996`) + commit-4 visual-review corrections (`8b53f296` — TwoSlopeNorm + unlit + saturated bwr) + 5 (orbit camera + UI `93abde78`) shipped on `dev`. Plan iter-1 locked + iter-1.3 (commit-3 head-architect calls) + iter-1.4 (commit-4 point-cloud coloring lock) + iter-1.5/1.6/1.7 (commit-4 visual-review corrections) + iter-1.8 (commit-5 head-architect calls — UI library, re-render mechanism, orbit camera) all banked. **Commit 5 visually reviewed + passed first try 2026-05-04** — orbit/pan/zoom working, side panel renders, dropdowns wired, geometry-only and scalar paths both correct. Branch strategy revised mid-commit-1 to single-branch flow per `feedback_single_active_branch`. **Next step: commit 6 (CLI flags — `--scalar`, `--colormap`, `--up`).**
...
**Commits 1 (scaffolding `02bceb9c`) + 2 (PLY load + `ViewerInput` `093f0410`) + 3 (geometry rendering `79b97339`) + commit-3 cold-read fixes (`b46d0b99`) + 4 (colormap pipeline `0c3ab996`) + commit-4 visual-review corrections (`8b53f296`) + 5 (orbit camera + UI `93abde78`) shipped on `dev`. Branch strategy revised mid-commit-1 to single-branch flow per `feedback_single_active_branch`. Commit 5 visually reviewed + passed first try 2026-05-04 — striking contrast with commit 4's three-iter correction cycle.**

1. Verify current state: on `dev`, commit `93abde78` is the most recent viewer commit. Working tree should be clean. `cargo test -p cf-viewer` should report 36/36 passing.
...
3. **Iter-2 still-open #9 (CI tests-debug propagation)** is now overdue (commits 3 + 4 + 5 all shipped without CI running cf-viewer's 36 unit tests). Wire cf-viewer into `.github/workflows/quality-gate.yml` `tests-debug` either as part of commit 6 or as its own commit before the arc merges.
