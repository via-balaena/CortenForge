...
- **2026-05-04 (iter 1.10 — commit-7 sphere-sdf-eval consumer migration + N+1 frontmatter coherence fix):** Commit 7 (`c47231a5`) shipped the smallest commit in the arc — README-only, no Rust touched. Visuals section's MeshLab two-filter chain (`Filters → Quality Measure and Computations → Per Vertex Quality Function`, `func q = signed_distance`, `Filters → Color Creation and Processing → Colorize by Vertex Quality`) plus the ParaView Coloring-panel aside both deleted; replaced by a single `cargo run -p cf-viewer -- <path>` invocation + auto-discovery + dropdown explanation + `--scalar=signed_distance` scripted-reproducibility alternative. 16 insertions, 11 deletions, net +5 lines.
  ...
  **Next iteration entry point:** user-call between (α) optional commit 8 (cf-viewer/README.md) and (β) sim-soft examples arc PR1 row 2 (`hollow-shell-sdf`). See Resume-here block. Sibling-commit candidate `src/main.rs:514` print_summary update is either folded into (α) as a tangential cleanup or queued for next sphere-sdf-eval touch as (β).
...
**Commits 1 (scaffolding `02bceb9c`) + 2 (PLY load + `ViewerInput` `093f0410`) + 3 (geometry rendering `79b97339`) + commit-3 cold-read fixes (`b46d0b99`) + 4 (colormap pipeline `0c3ab996`) + commit-4 visual-review corrections (`8b53f296`) + commit-4 cold-read doc fixes (`7ecbb54b`) + 5 (orbit camera + UI `93abde78`) + sphere-sdf-eval gradient_magnitude interlude (`8d9991c5`) + commit-5 cold-read fixes (`0245ef82`) + N+1 cold-read fixes (`7b6a4b1d`) + 6 (CLI flags `cb69ee56`) + sibling-CI tests-debug propagation (`a81cdcf5`) + 7 (sphere-sdf-eval README MeshLab → cf-view migration `c47231a5`) + N+1 frontmatter fix (`9fe022ed`) shipped on `dev`. Branch strategy revised mid-commit-1 to single-branch flow per `feedback_single_active_branch`. Commits 5 + 6 + 7 all visually reviewed + passed first try 2026-05-04 — third consecutive first-try success spanning input/lifecycle (5) + CLI/axis (6) + pure-documentation (7) classes. Iter-1 still-open #8 closed. PR-shape commit segmentation 7 of 7-8 done.**

1. Verify current state: on `dev`, the most recent commit is `9fe022ed` (N+1 frontmatter fix); the most recent code-changing viewer commit is still `cb69ee56` (commit 6). Working tree should be clean. `cargo test -p cf-viewer` should report 56/56 passing.
2. **User-call: which path next?**
   - **(α) Optional commit 8 — `cf-viewer/README.md`** documenting the binary's CLI surface + supported PLY shapes. Per PR-shape's optional commit. Small file, mostly cross-references + usage block. Tangential cleanup: fold the `src/main.rs:514` print_summary `MeshLab → cf-view` migration into this commit OR ship as its own sibling.
   - **(β) Resume sim-soft examples arc PR1 at row 2 (`hollow-shell-sdf`)** — see `sim/L0/soft/EXAMPLE_INVENTORY.md`. The viewer arc has been authored against ONE real consumer (sphere-sdf-eval); row 2 is the second consumer that surfaces refinements for cf-viewer. The `src/main.rs:514` cleanup queues for whatever sphere-sdf-eval touch happens next.
3. Sibling-commit candidate `src/main.rs:514` (`print_summary` println pointing at MeshLab / ParaView, half-migration risk surfaced at commit-7 cold-read) — user-call: ship as α-tangent cleanup, β-deferred polish, or its own immediate sibling. Trivial scope (5 println lines).
4. The Q7 mesh-v1.0 retrofit is its own followup PR after sim-soft PR1 finishes; that PR also picks up iter-1 still-open #5 (crease-angle splitting polish commit) before the 9 mesh README updates.
