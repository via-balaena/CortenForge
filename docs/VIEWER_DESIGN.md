...
**Status:** Commits 1 (scaffolding `02bceb9c`) + 2 (PLY load + `ViewerInput` `093f0410`) + 3 (geometry rendering `79b97339`) + commit-3 cold-read fixes (`b46d0b99`) + 4 (colormap pipeline `0c3ab996`) + commit-4 visual-review corrections (`8b53f296` — TwoSlopeNorm + unlit + saturated bwr) + commit-4 cold-read doc fixes (`7ecbb54b`) + 5 (orbit camera + UI `93abde78`) + sphere-sdf-eval gradient_magnitude interlude (`8d9991c5`) + commit-5 cold-read fixes (`0245ef82`) + N+1 cold-read fixes (`7b6a4b1d`) + 6 (CLI flags `cb69ee56`) + CI tests-debug propagation (`a81cdcf5`, sibling) shipped on `dev`. Plan iter-1 locked + iter-1.3 (commit-3 head-architect calls) + iter-1.4 (commit-4 point-cloud coloring lock) + iter-1.5/1.6/1.7 (commit-4 visual-review corrections) + iter-1.8 (commit-5 head-architect calls — UI library, re-render mechanism, orbit camera) + iter-1.9 (commit-6 head-architect calls — clap derive, seeded resources, UpAxis Resource + 3-way axis matrix) all banked. **Commits 5 + 6 both visually reviewed + passed first try** — second consecutive first-try success on an input/lifecycle commit, strengthening the calibration in `feedback_visual_review_is_the_test` (math-vs-render gap is specifically a viz-pipeline concern; commits that don't touch lighting/tonemap/colormap reliably pass first-try). **Iter-1 still-open #8 (CI tests-debug propagation) CLOSED 2026-05-04 via sibling commit `a81cdcf5`.** 56/56 cf-viewer unit tests (was 36 pre-commit-6 — +20 new across cli + mesh + camera + lib). Multi-scalar verification gap closed via the sphere-sdf-eval interlude (PLY now carries 2 scalars; dropdown switching visually verified). Branch strategy revised mid-commit-1 to single-branch flow per `feedback_single_active_branch`. **Next step: commit 7 (sphere-sdf-eval README MeshLab → cf-view migration).**
...
**Commits 1 (scaffolding `02bceb9c`) + 2 (PLY load + `ViewerInput` `093f0410`) + 3 (geometry rendering `79b97339`) + commit-3 cold-read fixes (`b46d0b99`) + 4 (colormap pipeline `0c3ab996`) + commit-4 visual-review corrections (`8b53f296`) + commit-4 cold-read doc fixes (`7ecbb54b`) + 5 (orbit camera + UI `93abde78`) + sphere-sdf-eval gradient_magnitude interlude (`8d9991c5`) + commit-5 cold-read fixes (`0245ef82`) + N+1 cold-read fixes (`7b6a4b1d`) + 6 (CLI flags `cb69ee56`) + sibling-CI tests-debug propagation (`a81cdcf5`) shipped on `dev`. Branch strategy revised mid-commit-1 to single-branch flow per `feedback_single_active_branch`. Commits 5 + 6 both visually reviewed + passed first try 2026-05-04 — second consecutive first-try success on an input/lifecycle commit (calibration confirmed). Iter-1 still-open #8 closed.**

1. Verify current state: on `dev`, the most recent viewer commit is `a81cdcf5` (sibling CI); the most recent code-changing viewer commit is `cb69ee56` (commit 6). Working tree should be clean. `cargo test -p cf-viewer` should report 56/56 passing.
2. Author **commit 7 — sphere-sdf-eval README MeshLab → cf-view migration**: update `examples/sim-soft/sphere-sdf-eval/README.md`'s "Visuals" section from the legacy MeshLab two-filter-chain instructions (Filters → Quality Measure and Computations → Per Vertex Quality Function `func q = signed_distance` then Filters → Color Creation and Processing → Colorize by Vertex Quality) to a single `cargo run -p cf-viewer -- examples/sim-soft/sphere-sdf-eval/out/sdf_grid.ply` invocation. Note the multi-scalar dropdown (gradient_magnitude alphabetical-first-pick + signed_distance via `--scalar=signed_distance` or runtime dropdown) and the divergent vs sequential auto-detect. End-to-end smoke test against the same fixture that motivated the arc.
3. Pause for user review; per-commit cadence per `feedback_one_at_a_time_review`. Visual review steps: (a) follow the README's instructions verbatim and confirm the screenshots match what cf-view actually renders; (b) verify the legacy MeshLab block is fully removed (no stale instructions); (c) cross-check the README's anchor list against the actual example's verify_* anchor names.
4. Continue down the 7-8-commit segmentation in the PR shape section above; commit 8 (optional) is the cf-viewer crate README. After commit 7 ships, the sim-soft examples arc PR1 can resume at row 2 (`hollow-shell-sdf`), authored against the now-stable cf-viewer.
5. The Q7 mesh-v1.0 retrofit is its own followup PR after sim-soft PR1 finishes; that PR also picks up iter-2 still-open #5 (crease-angle splitting polish commit) before the 9 mesh README updates.
