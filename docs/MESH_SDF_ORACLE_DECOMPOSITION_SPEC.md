...
**Status**: SPEC v2. **D.1 SHIPPED 2026-05-18 on dev `9b6863f6` + cold-read polish `8500da57`** (traits + parry oracles + cf-design blanket adapter + deprecated SignedDistanceField wrapper). **D.2 SHIPPED 2026-05-18 on dev `a9de1e62` + cold-read polish `ba0d1106`** (FloodFillSign + CachedGridSdf + 4 synthetic fixtures + numerical-equivalence test; +10 mesh-sdf tests, grade A, WASM clean). **D.3 SHIPPED 2026-05-18 on dev `9ec7462f`** — bundled D.3a + D.3b in one commit (181 insertions / 360 deletions across `sdf_layers.rs`, `insertion_sim.rs`, plus a one-line mesh-offset `#![allow(deprecated)]` shim deferring D.4a's deprecation cleanup). 138 cf-device-design tests pass on release; xtask grade A; clippy + WASM clean.
