...
**D.3 — cf-device-design migration.** Two commits.
- D.3a: `sdf_layers::build_cached_scan_sdf` adopts `CachedGridSdf::build`. Delete `Region` enum + `neighbours6` + the 3-region BFS code from sdf_layers (~150 LOC). The function's signature stays the same; downstream `extract_layer_surface` is unaffected.
- D.3b: `insertion_sim::build_grid_sdf` adopts `CachedGridSdf::build`. Delete `GridSdf` newtype (or keep as a thin wrapper for the public API) and the duplicate Region enum + BFS (~150 LOC). Preserve `GridSdfReport`'s public surface; it becomes a re-export / projection of `FloodFillReport`.
- Visual gate on iter-1 sock matches current state (cavity + Layer 0 + Layer 1 render unchanged).
- Insertion-sim regression test on a small fixture: tet bucketing matches pre-migration bit-equivalent.
- `cargo run -p xtask -- grade cf-device-design` ≥ A.

**D.4 — Other mesh-sdf consumers (mesh-offset + mesh-lattice).** Two commits.
- D.4a: `mesh-offset::offset_mesh` adopts `Signed<TriMeshDistance, FloodFillSign>` by default. `OffsetConfig` gains an enum `sign_oracle: { FloodFill, PseudoNormal }`. Default is `FloodFill` (safe). Tests cover both paths.
- D.4b: `mesh-lattice::infill` same migration. The existing inline comment (infill.rs:420-428) gets updated to reflect the new posture: ray-cast was ~36% wrong, face-normal heuristic was a workaround, FloodFillSign is the actual fix.
- `cargo run -p xtask -- grade mesh-offset` ≥ A.
- `cargo run -p xtask -- grade mesh-lattice` ≥ A.
