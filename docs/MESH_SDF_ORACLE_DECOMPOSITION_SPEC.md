...
- `SharedScanSdf` inner type swaps from `Arc<SignedDistanceField>` (= deprecated alias for `Signed<TriMeshDistance, PseudoNormalSign>`) to `Arc<Signed<TriMeshDistance, FloodFillSign>>`. All inherent methods (`new`, `mesh`) and the `impl cf_design::Sdf` preserve their external shape — derive.rs + lib.rs consumers see no API drift beyond the load_scan_sdf signature change below.
- `load_scan_sdf(path)` becomes `load_scan_sdf(path, bounds_padding_m, cell_size)`. lib.rs orchestrates by computing `cumulative_thickness + bounding_margin_m` (the same padding `derive_spec_and_ribbon` uses for SDF eval bounds) before the call, so the flood-fill grid encloses every point downstream queries. Spec recommendation #2 from cold-read entry §2 picked (option (a) — minimal API churn beyond the load signature).
- `derive.rs::derive_spec_and_ribbon`: `closed_sdf_arc` stays `Arc::new(scan_sdf.clone())` (SharedScanSdf is already flood-fill-signed); the explicit `flood_filled_sdf` call moved to scan.rs's `load_scan_sdf`. `open_sdf_arc` migrates to explicit `Signed { distance: TriMeshDistance::new(open_mesh)?, sign: PseudoNormalSign::from_distance(&distance) }` — sign-immune consumer (`UnsignedRindSdf` only reads `.eval(p).abs()`), so the cheap parry pseudo-normal path is correct here and avoids a second grid build.
- **Spec §D.5 #4 "drop open_sdf to bare `TriMeshDistance`" departure (small)**: bare `TriMeshDistance` doesn't impl `cf_design::Sdf` (only `mesh_sdf::UnsignedDistance`). The open SDF flows through `pinned_floor_shell<C, O>` whose `O: Sdf` bound requires a cf-design Sdf impl. Wrapping in `Signed<TriMeshDistance, PseudoNormalSign>` is essentially free (PseudoNormalSign reuses the Arc<TriMesh> via `from_distance`), preserves the cf-design Sdf contract, and is sign-immune downstream. Adding `impl Sdf for TriMeshDistance` in cf-design would be the alternative — banked for D.6 if any other consumer wants bare TriMeshDistance.
- Test helpers (`unit_cube_signed` in scan.rs, `unit_cube_sdf` in derive.rs) use cell_size 0.0173 m and 0.0017 m respectively — both **prime-ish so they don't divide the unit cube's 0.5 m / 0.05 m half-extents**. An aligned cell_size puts a lattice node EXACTLY on the cube surface, tags it Wall, and lets the BFS label expansion pick an arbitrary sign for that node — which then propagates to SharedScanSdf probes that round to it. Non-aligned cells keep every node off the surface so the sign at probe-distance > wall_threshold is unambiguously flood-reachable (Outside) or unreachable (Inside). Discovered during D.5 (initial cell_size 0.01 m exposed the failure on `derive_inset_shrinks_plug_and_shifts_layers_inward`'s +0.052 probe); banked as a reusable pattern for any future synthetic-cube test against a FloodFillSign-backed SDF.

Tests + gates: 44 lib + 6 integration pass (unchanged baseline). Clippy `-D warnings` clean. `cargo run -p xtask -- grade cf-cast-cli` = A across all 5 measurable automated criteria (Coverage / Layer / WASM rows inapplicable for the bin profile). Zero `SignedDistanceField` deprecation warnings remain in cf-cast-cli scope. Visual gate is user-side: run cf-cast-cli end-to-end on `~/scans/cast.iter1-design.toml` + `sock_over_capsule.cleaned.stl`; expected 4 piece STLs + 2 plug STLs + procedure.md, plug STLs watertight and visually resemble the cavity shape.
...
## Cold-read entry for next session (D.6)

D.5 is SHIPPED on dev `f74eb4c8`. cf-cast-cli now builds + grades A with no `SignedDistanceField` deprecation warnings in scope. Workshop iter-1 mold export still needs the **user-side visual gate** — run cf-cast-cli end-to-end on `~/scans/cast.iter1-design.toml` + `sock_over_capsule.cleaned.stl`; expected 4 piece STLs + 2 plug STLs + procedure.md, plug STLs watertight (no open edges) and visually resemble the cavity shape. If the visual gate clears, the D arc's load-bearing claim is validated and D.6 closes out the spec.

D.6 is the final docs + memory sub-leaf. Scope:

1. **Spec memo annotation**. Update `project_mesh_sdf_oracle_decomposition_spec.md` with the D.5 SHIPPED row (as-built table + headline + the cell_size-alignment pattern). Promote D.5's `cell_size` non-divisor pattern to a banked pattern memo.
2. **Workspace examples batch-migration**. Per the D.4 stress-test finding (and D.4a as-built), the following examples still consume `SignedDistanceField`:
...
4. **Memory: cf-cast-cli watertight-discovery memo** ([[project-cf-cast-plug-layer-0-watertight-discovery]]). Mark RESOLVED with pointer to D.5's commit + visual-gate outcome.
5. **Memory: MEMORY.md Resume note**. Rewrite to point at either workshop iter-1 physical cast (if visual gate clears) OR the B.0 audit (cf-scan-prep cap-winding fix; separate arc).
6. **Optional perf followup**: expose `CachedGridSdf::into_signed_values() -> Vec<f64>` (or `as_signed_values() -> &[f64]`) + a `ScalarGrid::from_values(...)` constructor on the mesh-offset side. Halves the cf-device-design closed-grid build wall-clock by skipping the trilinear-sample second pass. ~10 LOC on the mesh-sdf side, smaller on the consumer side. Spec §"Banked: D.6 perf followup" has the full reasoning. Surface as a decision question — could be its own commit, or skipped if perf isn't load-bearing yet.
7. **Cold-read pass after D.6 ships**, per [[feedback-cold-read-review-post-ship]] (5th demonstration of the pattern across D.1 / D.2 / D.3 / D.4). Bundle any findings into a polish commit.

The visual-gate posture per [[feedback-bookmark-when-surface-levers-exhaust]]: if the iter-1 plug STLs are non-watertight or visually wrong despite the FloodFillSign migration, STOP and bookmark — don't claim D arc success.
