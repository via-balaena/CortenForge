...
## Cold-read entry for next session (D.5)

1. Re-read §6 row #3 + #4 (per-consumer migration table) + §"Sub-leaf ladder" D.5 in this spec. Also re-read the D.4a + D.4b SHIPPED entries — same migration shape (drop `SignedDistanceField`, swap to `flood_filled_sdf` with explicit bounds + cell_size), and D.4b's sign-convention-flip discovery (mesh-offset's inward-MC inverted-winding meant the consumer needed a closure-side negation that flood-fill makes redundant) likely applies to UnsignedRindSdf inside cf-cast-cli too — verify before assuming.
2. Read `tools/cf-cast-cli/src/scan.rs:15-180` (`SharedScanSdf` wraps `SignedDistanceField`). Two migration shapes to choose between: (a) keep `SharedScanSdf` as the consumer-facing type, swap its inner `Arc<SignedDistanceField>` to `Arc<Signed<TriMeshDistance, FloodFillSign>>`, preserve the inherent method delegation pattern. (b) rename + restructure to expose the explicit composition. Pick (a) — minimal API churn; D.6 can address renames.
3. Read `tools/cf-cast-cli/src/derive.rs:200-260` (`derive_spec_and_ribbon` — the `closed_sdf` and `open_sdf` construction sites). `closed_sdf` migrates to FloodFillSign (this is the load-bearing change — the parry-pseudo-normal wrong-sign at +Y/+Z far-field on iter-1 sock is what's failing the watertight gate on `plug_layer_0.stl`). `open_sdf` is consumed only via `UnsignedRindSdf` (which calls `.unsigned_distance()` / `.abs()`); migrate to bare `TriMeshDistance` (sign-immune; saves the flood-fill grid build cost). Bounds for the flood-fill: spec §D.5 says "reuse `sdf_bounds` already computed" — locate the existing AABB and reuse it.
4. Read `design/cf-design/src/solid_layered.rs` `UnsignedRindSdf` impl — verify it consumes `.abs()` / `.unsigned_distance()` only (no sign-dependent calls). If a sign-dependent call slipped in, the spec's "drop to TriMeshDistance" recommendation needs revisiting.
5. **Stress-test the visual gate hypothesis BEFORE committing.** The watertight failure was discovered on iter-1 sock at `plug_layer_0.stl` (`3156 open edges`). The forensic far-field probe from [[project-cf-cast-plug-layer-0-watertight-discovery]] showed +Y / +Z asymmetric wrong-sign. Confirm that the FloodFillSign migration would produce different signs at those probes — re-read the postmortem and/or write a quick test against the iter-1 cleaned mesh fixture.
6. Visual gate post-D.5: run cf-cast-cli end-to-end on iter-1 sock fixture (`~/scans/sock_over_capsule.cleaned.stl + .prep.toml`). Expected: 4 piece STLs + 2 plug STLs + procedure.md in `~/scans/cast_iter1_design/` (or wherever the example writes) in single-digit minutes. Plug STLs should be watertight (no open edges via `cargo run -p mesh-stl-info -- <path>` or whatever the validator is) and visually resemble the cavity shape. User runs the visual check.
7. Run `cargo run -p xtask -- grade cf-cast-cli` ≥ A.

**Implementation start (D.5)**: `tools/cf-cast-cli/src/scan.rs` (`SharedScanSdf` migration) → `tools/cf-cast-cli/src/derive.rs::derive_spec_and_ribbon` (closed_sdf + open_sdf migrations) → `cargo test -p cf-cast-cli` → end-to-end iter-1 mold export → user-side visual gate.
