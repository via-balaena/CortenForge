...
**Predecessor docs:** [`VIEWER_DESIGN.md`](VIEWER_DESIGN.md) (cf-view Q1–Q8), [`sim/docs/ARCHITECTURE.md`](../sim/docs/ARCHITECTURE.md), [`sim/L0/soft/EXAMPLE_INVENTORY.md`](../sim/L0/soft/EXAMPLE_INVENTORY.md)
...
- ✅ faer LU fallback (A2, shipped 2026-05-11 in commit `dda6cf7c`; resolves the capsule-cap apex non-PD Llt pivot via the `FactoredFreeTangent` factor wrapper (its `FactorInner` enum carries the `Llt` happy-path / `Lu` fallback) routing both factor sites through `factor_free_tangent` — acceptance is lib-owned in `backward_euler/tests.rs` (`factor_free_tangent_falls_through_to_lu_on_non_pd` + siblings). Row 22/25 deeper-penetration walls remain validity-bound, NOT Llt — A2 does not unblock those. The `scan-fit-3layer-sleeve-v15` demonstrator that once exercised the vertex-13271 trip was deleted in arc-close b, 2026-07-17: NH's σ=2.0 validity wall, a separate limit from A2, makes it hard-RED with no wider bound.)
...
- ✅ k-nearest-tet weighted scalar interp (C2.1 categorical-stays-integer dispatch — `_id` → nearest-tet, k=1, position-based winner on slab_cut; C2.2 Shepard kNN-IDW continuous smoothing on `design_surface` with k=8, w=1/(d²+ε); `design_slab_cut` C2.1 gap-fix added in the C2.2 commit)
...
| A2 | faer LU fallback for SPD pivot failure | Resolves the capsule-cap apex non-PD Llt pivot (Llt→Lu at the two condensed-tangent factor sites); latent enabler for B2/B4 finer-cell layered-field cases. Does NOT unblock row 22/25 deeper penetration — those walls are validity-bound (NH/Yeoh `max_stretch_deviation`), upstream of any factorization |
...
| **S2 — MVP image, visually complete** | A2 + B2 + C2 + D2 | Indefinite-tangent contact survival + finer cells + smooth scalar + scrub controls — **MVP target image lands here**. Row 25 depth stays capped at 4 mm (validity-bound wall, separate followup); A2 covers the capsule-cap apex non-PD pivot case |
...
- **S2 (partial)** — A2 arc on dev, 2026-05-11. Shipped: **A2** (faer LU fallback at the two condensed-tangent factor sites in `CpuNewtonSolver`). New `pub(crate)` `FactoredFreeTangent` factor wrapper whose `FactorInner { Llt(_), Lu(Box<_>) }` enum forwards `solve_in_place_with_conj`; happy path bit-identical to pre-A2 code, fall-through engages on `LltError::Numeric(NonPositivePivot)` and factors via Lu against a cached `SymbolicLu`. Acceptance is lib-owned in `backward_euler/tests.rs` (`factor_free_tangent_falls_through_to_lu_on_non_pd` + `..._llt_path_on_spd_matrix` + `..._lm_bump_rescues_non_pd`). It was originally demonstrated on a sibling crate `scan-fit-3layer-sleeve-v15` (row 21.5): pre-A2 the v1.5 capsule + capsule geometry was PARKED with a documented Llt non-PD pivot at Newton iter 2-3 (vertex 13271 at 1 mm); post-A2 the fallback engaged 4× at iters 2-5 (vertex 13271 at iter 2 matched the parked memo exactly), Newton converged in 92 iters to 9.05e-11. That demonstrator was later DELETED (arc-close b, 2026-07-17) — it went hard-RED on NH's σ=2.0 validity wall (a separate limit from the A2 Llt pivot, with no wider bound for Neo-Hookean); the LU-fallback acceptance survives in the lib tests above. B2 + C2 + D2 still ahead for S2 completion. Commits: `415239b4` (A2 framing in roadmap), `b85e05a3` (SymbolicLu cache), `dda6cf7c` (Lu fallback + enum + unit tests), `3c116fda` (row 21.5 unpark).
- **S2 (partial)** — C2.2 kNN-IDW continuous-scalar smoothing + design_slab_cut C2.1 gap-fix on dev, 2026-05-12. Shipped: **C2.2** (Shepard-style kNN-IDW for `design_surface`'s continuous-scalar path — replaces the pre-C2.2 vol-weighted-per-vertex + barycentric pipeline with `idw_k_nearest_tet_centroids(probe, mesh, positions, candidates, per_tet_scalars, k=8, eps=(cell_size×1e-3)²)`, reusing the existing `TetGrid` 3×3×3 candidate pool; categorical path from C2.1 unchanged). Module-level `const IDW_K: usize = 8` documents the smoothing radius (matches the typical tets-per-cell on a BCC analysis mesh). **Plus**: C2.1 gap-fix on `design_slab_cut` — that primitive had been missed by the C2.1 dispatch and was still routing `_id` scalars through vol-weighted-vertex + barycentric (cf-view Q5 detector flipped to Sequential viridis at every shell boundary). Added the same categorical/continuous split `design_surface` uses, with `nearest_tet_centroid_idx` over the existing `slab_tets` filter as the categorical candidate pool. 4 new viz tests (IDW constant-field / single-candidate / symmetric-two-candidate + `design_slab_cut` categorical integer-only). Eyes-on-pixels row 25: `sleeve_design_slab_cut_x0_final.ply` now renders 3 cleanly separated shells as tab10 categorical; `sleeve_design_surface_final.ply` ψ field shows modest visible smoothing at the high-strain contact ring (the data's 5-order-of-magnitude range limits visibility on the body bulk — banked "log-scale ψ colormap in cf-view" as a separate followup that would unlock seeing the kNN-IDW effect on the bulk material). C2.x mode-of-K-nearest polish for categorical (banked from C2.1) still on the docket if the BCC stair-step at shell boundaries needs further work.
- **S2 (partial)** — C2.1 categorical-scalar dispatch on dev, 2026-05-12. Shipped: **C2.1** (suffix-`_id` categorical/continuous split in `sim_soft::viz` primitives). New helpers `is_categorical_scalar_name` (suffix `_id`) + `nearest_tet_centroid_idx` (k=1 over a candidate pool). Per-primitive dispatch: `boundary_surface` picks the largest-volume incident tet per analysis-mesh vertex; `slab_cut`/`slab_cut_deformed` pick the closest-centroid-to-cut-vertex tet via a 3D position-based winner on edge dedup (replaces the pre-C2.1 prototype's largest-volume-tet rule that revealed BCC stair-step at every shell boundary); `design_surface` (+ deformed/scene siblings) does a single nearest-tet-centroid lookup per probe shared across all categorical scalars, reusing the existing `TetGrid` 3×3×3 cell window. Continuous scalars (`psi_j_per_m3`, …) keep the existing volume-weighted/barycentric/linear-interp pipeline bit-exact (regression-pinned by a new mixed-scalar test). Eyes-on-pixels on row 25 `slab_steps/` confirms clean 3-shell tab10 categorical separation; cf-view's Q5 detector now correctly picks Categorical instead of falling back to Sequential viridis at every shell/zone boundary. Residual BCC-lattice stair-step at shell boundaries is mesh-structure visible through k=1 sampling — banked as a C2 polish followup (mode-of-K-nearest, or A3 adaptive BCC upstream). C2.2 (kNN-IDW continuous smoothing) + B2 + D2 polish still ahead for S2 completion.
