...
> ⛔ **The v1 single-piece cast path was retired 2026-09-12 (`5515e84e`).**
> `CastSpec::export_molds`, `CastSpec::write_procedure` and the
> `layered-silicone-device-v1` example no longer exist; production is
> `CastSpec::export_molds_v2`. Sections below that describe them are a
> **record of what shipped in May 2026, not runnable instructions.**
>
> Why: `clip_above_body` cut tangent to a curved body, leaving a cup lip that
> tapered to zero thickness (measured 0.000 mm at r=4.21 → 1.192 mm at r=7.62 on
> the example's capsule dome). The F4 ThinWall detector was right to refuse it.
> The only CI-covered caller used a *cuboid* body, whose top face is parallel to
> the clip plane, so no feather edge could form — the fixture could not produce
> the class the gate exists to catch.
...
⛔ **SUPERSEDED — this crate was deleted 2026-09-12 (`5515e84e`); the command below no longer runs.** Recorded as the May 2026 result. Generated by `cargo run --release -p example-cast-layered-silicone-device-v1`; landed in `examples/cast/layered-silicone-device-v1/out/`:
...
- `CastSpec::export_molds(out_dir, printer_config) -> Result<MoldExportReport>` — writes STLs, runs printability gate. ⛔ *retired 2026-09-12; superseded by `export_molds_v2`.*
- `CastSpec::write_procedure(path) -> Result<()>` — writes the markdown procedure spec including per-layer pour volumes. ⛔ *retired 2026-09-12; superseded by `write_procedure_v2`.*
