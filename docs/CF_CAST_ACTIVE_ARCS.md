...
- **Recon:** `docs/CF_CAST_SEAM_FLANGE_RECON.md` (+ cold-read
  pass-1 applied).
- **State:** **S1 shipped (`8997bdd3`) + cold-read pass-1 (`746a75bc`) + S2 shipped (`3f50689e`) + S3 shipped (`9574f46f`) + S3 cold-read pass-1 (`a4af6b25`) + S3 cold-read pass-2 (`e757551d`) + S4 measured (no code commit, 2026-05-25).** S5 pending (gated on workshop cf-view smoke).
...
- **Phases (§F-13):**
  ...
  - S4 — ✅ MEASURED 2026-05-25 (no code commit). Production iter-1 regen on `~/scans/cast.toml` → `~/scans/cast_iter1_post_flange_smoke/`. Wall 1:46.83 = +2.6% over 1:44.13 baseline (cumulative 18.9× over pre-parallel-meshing 33.6 min). 14 STLs emit (6 cup pieces WITH flange + 3 plugs + 1 funnel + 1 platform + 3 gasket molds). §R1 connectivity gate PASSES on all 6 cup pieces; F4 gate PASSES. Cold-read pass-1 + pass-2 prose fixes all verified intact in rendered procedure.md (35.7 KB). Cup-piece face counts +52-65% from flange material; all other STLs unchanged.
