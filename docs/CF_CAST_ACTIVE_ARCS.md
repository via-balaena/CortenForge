...
- **Recon:** `docs/CF_CAST_SEAM_FLANGE_RECON.md` (+ cold-read
  pass-1 applied).
- **State:** **S1 shipped (`8997bdd3`) + cold-read pass-1 (`746a75bc`) + S2 shipped (`3f50689e`) + S3 shipped (`9574f46f`).** S4-S5 pending.
...
- **Phases (§F-13):**
  ...
  - S3 — ✅ SHIPPED (`9574f46f`). procedure.rs `## Cup-Half Clamping with Gasket Installation` section with 4-arm (FlangeKind, GasketKind) prose dispatch. (Plate, Mold) iter-3-default branch emits 8-step workshop protocol (gasket pour → peel → position inside flange perimeter → clamp 4 quadrants hand-tight + 1/8 turn → silicone pour → cure → release); 3 partial-enable / fallback branches emit tailored guidance. 238 cf-cast lib / clippy / fmt clean; 2 new doc-anchoring tests paired with negative-assertions to catch dispatch fallthrough.
