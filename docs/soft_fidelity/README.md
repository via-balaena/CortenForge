...
| **M2 — Roller BCs + multi-material + mode recon** *(active)* | software, published-data oracle | Per-axis (roller) Dirichlet BCs (unblocks the Layer-2 keystone + a genuine free-lateral traction coupon that re-discovers λ_t — the free-lateral solve M1's kept patch test stood in for); Ecoflex 00-10 validated through the same machinery (the one other real curve in our oracle); a mode-oracle recon deciding whether simple-shear/equibiaxial (b) is M2's deep arc or defers to M3. Near-incompressibility (e) stays a measured trigger. Recon: `03_phases/m2_roller_bc_and_modes/recon.md` |
