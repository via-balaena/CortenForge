...
3. **`SdfMeshedTetMesh` → `Tet10Mesh` bridge inside `insertion_sim`**, plus
   ...
   **failed on Linux/x86 CI**, where the identical commit drove the residual
   ...
   ⭐ **Every number in this section is regenerated by three commands.** They
   are the referent: a figure here that the probes no longer print is a figure
   that has drifted, and one quoted without re-running them is a claim about a
   run nobody did — which is how a `1.003×` that exists in no source file got
   into an earlier revision of this section.
