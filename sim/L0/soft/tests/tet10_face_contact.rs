...
use sim_soft::{
    ...
    SphereSdf, Tet10Mesh, TranslatedSdf, Vec3, VertexId,
};
...
// A CURVED indenter (rung 8c gate): a sphere below the block whose apex sits
// `SPHERE_STANDOFF` under the block's bottom face. `d̂` is widened so the whole
// (flat) bottom face stays in the barrier band despite the sphere's curvature
// drop-off, and the radius is comparable to the block so `∇²sd ≈ 1/R` is a
// non-negligible fraction of the barrier tangent — the term 8c adds.
const SPHERE_R: f64 = 0.15;
const SPHERE_D_HAT: f64 = 0.05;
const SPHERE_STANDOFF: f64 = 0.02;

/// Sphere centred on the block's bottom-face centre, `SPHERE_R + SPHERE_STANDOFF`
/// below it, so the apex is `SPHERE_STANDOFF` under the bottom face.
fn sphere() -> TranslatedSdf<SphereSdf> {
    let half = 0.5 * EDGE;
    TranslatedSdf {
        inner: SphereSdf { radius: SPHERE_R },
        offset: Vec3::new(half, half, -(SPHERE_R + SPHERE_STANDOFF)),
    }
}
...
#[test]
fn tet10_face_contact_curved_sphere_reaction_jvp_matches_resolve_fd() {
    // Rung 8c gate — the CURVED-indenter counterpart of the plane reaction-JVP.
    // 8b's gate used a PLANE precisely because `∇²sd = 0` hides the `b'·∇²sd`
    // Hessian term. Against a sphere the assembled tangent `A_ff` carries that
    // term, and the re-solve FD reconstructs the TRUE tangent through the root
    // (the residual is curvature-free, so the converged root is tangent-agnostic)
    // — so a dropped/wrong curvature term makes analytic ≠ FD. This is a genuine
    // modeling gate, not a shared-parameter FD (the curvature lives only in the
    // tangent, and the FD side gets the true tangent via the root).
    let cube = HandBuiltTetMesh::uniform_block(N, EDGE, &field());
    let mesh = Tet10Mesh::from_tet4(&cube);
    let rest = rest_dofs(&mesh);
    let n = rest.len();
    ...
    cfg.dt = STATIC_DT;
    cfg.max_newton_iter = 80;
    let contact = IpcRigidContact::with_params(vec![sphere()], KAPPA, SPHERE_D_HAT);
    let solver = CpuNewtonSolver::new(Tet10, mesh, contact, cfg, bc);
    ...
    // Press the pinned top down so the bottom face is firmly on the sphere cap.
    let mut x_op = rest;
    for &v in &top {
        x_op[3 * v as usize + 2] -= 0.3 * SPHERE_D_HAT;
    }
    let x1 = solve(&x_op);
    ...
    let mag = pinned_dofs.iter().map(|&i| an[i].abs()).fold(0.0, f64::max);
    eprintln!("Tet10 curved-sphere face-contact reaction JVP: ‖·‖_∞={mag:.3e} best_rel={best:.3e}");
    assert!(
        mag > 1.0,
        "reaction sensitivity implausibly small (contact inactive?)"
    );
    assert!(
        best < 1e-5,
        "Tet10 curved face-contact reaction JVP disagrees with re-solve FD: best_rel={best:e}",
    );
}
