...
use sim_soft::{
    BoundaryConditions, CpuTet4NHSolver, CpuTet10NHSolver, InitialGuess, LoadAxis, MaterialField,
    Mesh, NewtonStep, NullContact, Solver, SolverConfig, SolverFailure, Tet10Mesh, VertexId,
...
const EI_TARGET: f64 = 310.0;
/// Mass per unit length (kg/m) — a ~400 g stick over ~1.6 m.
const MASS_PER_LENGTH: f64 = 0.25;
...
fn free_dof_of(arm: Arm, nx: usize, ny: usize, nz: usize) -> usize {
...
const SLAPSHOT_DEFLECTION: f64 = 0.10;
...
/// predictor ([`where_the_dynamic_rig_loses_convergence`]). Ramped, it clears:
/// **Tet4 needs 6 frames, Tet10 needs 60** — the quadratic element is `10×` less
/// tolerant of an abrupt load change
/// ([`whether_load_continuation_clears_the_wall`]). `60` is the value both arms
...
fn one_frame<S: Solver>(
    ...
    let step = solver
        .try_replay_step(&x_in, &v_in, theta, FRAME_DT)
...
fn describe_failure(e: &SolverFailure) -> String {
    match e {
    ...
    match arm {
        Arm::Tet10 => {
            ...
            for k in 0..RAMP_FRAMES + MEASURED_FRAMES {
                ...
                match one_frame(&solver, &mut x, &mut v, &theta) {
                    Ok((it, t)) => {
                        if k >= RAMP_FRAMES {
                            iters.push(it);
                            ms.push(t);
