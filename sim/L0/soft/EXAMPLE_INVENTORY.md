...
- **2026-05-06 (iter 9 — C2c shipped: `sim/L1/sim-bevy-soft/` skeleton crate):** Pre-author deep-architecture session covered eight sub-decisions; user requested a re-think on three (Q3 trajectory carry, Q6 bevy features, Q8 normals) per `feedback_genuine_agreement` "lets talk" pattern banked iter-8 (a). All three flipped from default after second-pass thinking — the deeper-architecture answer beat the minimum-diff status quo in each case.
  ...
  **Next iteration entry point:** C2c-N+1 cold read for doc-rot fixups (5+ items expected per iter-8's "code-moves-to-shared-crate doc-rot" pattern, scaled to "new-shared-crate-births-with-multiple-modules" — sweep `lib.rs`, `mesh.rs`, `trajectory.rs`, `plugin.rs` docstrings for predecessor-crate-history references, redundant phrasing, and stale plan claims in module docs). After C2c-N+1: C2d sim-bevy migration to cf-bevy-common (read `sim/L1/bevy/src/{camera,plugin,lib}.rs` for migration surface; resolve the three reconciliation points banked iter-7 — defaults → cf-viewer's broader bounds win, sim-bevy's `spawn_orbit_camera` overrides via builders so view stays unchanged; pan guards → cf-viewer's safer version; framing_for_aabb → shared, sim-bevy may use it).
