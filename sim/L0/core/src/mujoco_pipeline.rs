...
// Re-imports from forward module (Phase 8a extraction).
// These functions were extracted but are still called from within the monolith
// (integrate(), compute_muscle_params()). Re-imported here; removed in Phase 12.
use crate::forward::mj_fwd_position; // monolith: removed in Phase 12
use crate::forward::mj_next_activation; // monolith: removed in Phase 12
...
use crate::jacobian::{mj_differentiate_pos, mj_integrate_pos_explicit}; // called by integrate(), mj_runge_kutta()
#[cfg(test)]
...
pub(crate) fn mj_runge_kutta(model: &Model, data: &mut Data) -> Result<(), StepError> {
    // Butcher tableau
    const RK4_A: [f64; 9] = [
