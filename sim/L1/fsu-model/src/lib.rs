...
/// |---|---|---|---|---|
/// | 0.05 | **stalls** | **stalls** | 0.126 | 0.098 |
/// | 0.08 | drives | **stalls** | 0.131 | 0.100 |
/// | 0.10 | drives | **stalls** | 0.136 | 0.101 |
/// | 0.15 | drives | drives | 0.146 | 0.104 |
/// | 0.20 | drives | drives | 0.156 | 0.107 |
/// | **0.25** | **drives** | **drives** | **0.170** | **0.111** |
/// | 0.40 | drives | drives | 0.242 | 0.141 |
///
...
///
/// ★★ **And the lofted columns are BIT-IDENTICAL across α.1** — every drivability verdict, and
/// `assert_floor_is_conditioning_only`'s spreads to four decimals (−24.8449 … −24.8269 and
/// −22.3225 … −22.2903). That is the two-geometry design of this table paying off exactly as
/// intended: the lofted disc is *fabricated*, so it never went through the scanned-SDF path α.1
/// fixed, and it therefore acts as an untouched control. The change is isolated to the scanned
/// columns, which is evidence the fidelity move is the geometry fix and not a solver-wide drift.
///
...
/// shipped row now reads **0.111 mm RMS at 91.0 % delivered** (it read 0.767 mm at 67.4 % before
/// rung β, and 0.694 mm at 67.9 % when this table was built). Read the rows as the *comparison
/// between floors* they were built to be, not as current absolutes.
///
