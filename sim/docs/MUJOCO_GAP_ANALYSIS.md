...
- **3A-v Constraint/Joint + Physics** (#38–41, §40a–e, §42A-i–iii): ~~ball/free joint limits~~ ✅, ~~`wrap_inside`~~ ✅, ~~fluid forces~~ ✅, ~~fluid derivatives (§40a)~~ ✅, ~~fluid sleep filtering (§40c)~~ ✅, ~~tendon visualization data (§40b)~~ ✅, `disableflags`, sparse fluid Jacobian (§40d), `mj_jac_point` refactor (§40e), flex edge Jacobian (§42A-i), flex rigid flags (§42A-ii), flex pre-computed fields (§42A-iii) (~~#42 flex/flexcomp~~ subsumed by §6B)
