...
| R-1.SD-4 | SD-4 surfaces ml-chassis test fragility | **Done in PR** | First commit-1 CI run on x86_64 ubuntu surfaced `artifact::tests::round_trip_mlp_policy` failing with 2 ULP forward-output divergence. Root cause: `MlpPolicy::forward_with_hidden` uses `mul_add`; on x86 with auto-vectorization, two independently-allocated `Vec<f64>` policies produce 1–2 ULP differences (ARM `vfmla` always fuses, hiding it on dev box). Compounded by serde_json's f64 round-trip introducing sub-ULP drift on some values. Fixed inline at `sim/L0/ml-chassis/src/artifact.rs:904` by replacing `assert_eq!(forward_outputs)` with a tolerance loop (`< 1e-10`); test invariant tightened to "save/load preserves forward output to numeric precision," which is what the artifact contract actually guarantees. SD-4 working as intended — exposed a real over-specified test on the first run. |
