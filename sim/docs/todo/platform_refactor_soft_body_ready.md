...
**Status:** E.1, E.2, E.3, E.4 locked. E.5 remaining (Group E close + consolidated A.1-line-78 footnote covering all four named-consumer outcomes). Cadence shifted by walk shape: argue-and-refine for E.1/E.2/E.4 placement decisions (sim-opt + sim-therm-env + sim-rl baselines — well-trodden CEM/REINFORCE/PPO/TD3/SAC, shallower specialist depth); recommend-first for E.3 autograd-internals specialist-depth territory; info-asymmetry override applied throughout. Option-B verification cadence (grade + broader consumer-surface grep) applied before each lock — direction flips by walk shape (consumer→upstream for E.1/E.2/E.4 external crates; upstream→downstream for E.3 chassis-internal walk). Hygiene-commit-then-decision pattern firmly conditional on F-class trigger (5-walk pattern: D.4.b + E.1 + E.2 applied; E.3 + E.4 NOT applied — both grade A across all 6 criteria, no prelude). Pattern is conditional, not default-apply.
...
**E.4 does not lock:**
- E.5 Group E close — `platform_refactor_soft_body_ready.md` line 78 footnote consolidating all four named-consumer outcomes; updated Group E status line; review of any audit-driven items deferred to Group F or A.4 §1 sweep candidate. Single-session close expected.
