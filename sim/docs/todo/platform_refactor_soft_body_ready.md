...
**Status:** E.1, E.2, E.3 locked. E.4–E.5 remaining (sim-rl baselines, then Group E close). Recommend-first cadence held across all three walks (E.3 especially: autograd-internals specialist-depth territory); Option-B verification cadence (grade + broader consumer-surface grep) applied before each lock; hygiene-commit-then-decision pattern applies when F-class triggers fire (E.1 for sim-opt's dep-chain clippy bleed, E.2 for sim-therm-env's own Cargo.toml dep-justification gap) and does NOT apply when grade is clean (E.3, grade A across all 6 automated criteria — no prelude).
...
**A.1-line-78 third overstatement.** A.1's "Downstream work (Group E)" paragraph at line 78 names `AutogradPolicy`/`AutogradValue` forward signatures as Tensor-site consumers. Reality verified by grep (`rg "Tensor" sim/L0/ml-chassis/src/autograd*.rs` → zero hits): **zero `Tensor` sites in any E.3 module** — forward signatures use `&[f32]` obs + `&[f64]` params/actions throughout. Third consecutive overstatement in the line-78 prediction (E.1 sim-opt = zero; E.2 sim-therm-env = partial hold at 4 prod + ~28 test; E.3 autograd = zero). E.5-close footnote now covers all four named consumers.
