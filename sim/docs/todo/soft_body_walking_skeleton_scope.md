...
| SD-2 | `sim/L0/soft/src/readout/scene.rs` | Step-3b commit (`Tet4` + `SingleTetMesh`) did not add `SoftScene::one_tet_cube()` constructor. Bundling it here would force `SceneInitial` population + `SolverConfig` stubs ahead of the Solver commit, mixing per-commit concerns. Closes with §9 step 4 Solver commit (if the solver's config doubles as scene-assembly) or §9 step 6 ForwardMap-composition commit (if scene assembly is its own layer), whichever consumes it first. Non-blocking for step 3b — §7 green-checklist items satisfied by the Tet4 FD test alone. | Scaffold deferral |

BF-1..BF-5 are not skeleton-blocking — the skeleton implements from the underlying math + the verified faer/chassis APIs, stubbing book methods whose shape doesn't match shipped infrastructure. They're book-edit backlog for whenever Pass 2/3 touches these chapters. SD-1 and SD-2 close inside the skeleton PR sequence.
