...
- [ ] `cargo tree -p sim-soft` — no C/C++ compile step **added to the graph by sim-soft's direct deps** (wgpu + faer + nalgebra + sim-ml-chassis are pure-Rust); pre-existing transitive chain `sim-ml-chassis → sim-mjcf → mesh-io → zip → zstd-sys` still pulls `cc` as a build-dep — see SD-5 below for the committed follow-up audit (I-6a).
...
| SD-5 | `sim/L0/ml-chassis/Cargo.toml` → `sim-mjcf → mesh-io → zip → zstd-sys` | `cargo tree -p sim-soft` surfaces `cc v1.2.57` as a build-dep through the transitive chain `sim-soft → sim-ml-chassis → sim-mjcf → mesh-io → zip → zstd → zstd-sys`. I-6a's "no C/C++ compile step **added to the graph**" is met (skeleton didn't add it); §7 item 7 was originally phrased "no C/C++ build step / pure-Rust chain" — tightened by this commit to match I-6a literally. Underlying question — why does the autograd chassis depend on MJCF at all? — is platform-infra scope, not skeleton work. **Committed follow-up:** new audit session on a separate branch off main after the skeleton PR merges; the audit decides whether the dep is load-bearing (no change) or severable (refactor PR). | Platform-infra audit, deferred |
