...
- [ ] `cargo tree -p sim-soft -e normal` — no C/C++ compile step in the release build graph; no `sim-mjcf`, `mesh-io`, `zip`, `zstd-sys`, or `cc` transitively. Literal pure-Rust claim (I-6a) — previously weakened to "no C/C++ added by sim-soft's direct deps" while SD-5 was open; SD-5 closed by PR relocating stock task factories from `sim-ml-chassis` to `sim-rl` and moving `sim-mjcf` to chassis `[dev-dependencies]`.
- [ ] No `unsafe` in sim-soft production code (grader enforces; sanity-check).
- [ ] Gradcheck relative error ≤ 1e-5 per component on all 4 θ axes (1 Stage-1 + 3 Stage-2), verified by `stage_1_gradcheck_central_fd` + `stage_2_gradcheck_central_fd` in `invariant_4_5_gradcheck.rs` (I-4 + I-5).
...
| R-5 | `NewtonStepVjp` sign convention per residual form "inertia + internal − external" | Part 6 Ch 02:13–15 derivation; gradient sign follows. | **✓ Round 1 — verified.** Part 6 Ch 02:13–15 gives $\partial x^* / \partial \theta = -A^{-1}\, \partial r / \partial \theta$ at $r = 0$; spec §5 VJP formula is mathematically correct. Book code sketch at Ch 02:34 is pseudocode-only (see BF-3). **✓ Round 2 — operationally validated (step 5).** `tests/invariant_4_5_gradcheck.rs::stage_1_gradcheck_central_fd` passes IFT-vs-FD at 5-digit relative-error bar for $L = x_\text{final}[11]$, θ = 10 N. |
...
- **S-4 — `parent_cotans` accumulation at physics scale.** **✓ RESOLVED 2026-04-23 (Round 1).** Read `sim/L0/ml-chassis/src/autograd.rs:769-782` (the `BackwardOp::Custom(op)` arm inside `Tape::backward`). Slots ARE zero-initialized with parent's stored shape via `Tensor::zeros(self.values[p as usize].shape())` — the tape looks up the parent's shape from the value it was pushed with, so `param_tensor` / `push_custom` / elementwise ops all get shape-correct cotangent slots. Accumulation is `ew_add_assign` (matches `+=` contract). Fan-in via duplicate parent indices works (each slot contribution accumulates independently before flush to `self.grads[p]`). Backward loop `for i in (0..=out_idx).rev()` is deterministic — no RNG, no parallelism, no clock. **Minor note:** line 750 skips nodes with all-zero cotangent — optimization, not correctness issue, but means a custom op whose upstream cotangent happens to be identically zero silently won't run its VJP. Not a skeleton concern. **Round-2 follow-on (2026-04-23, step 5):** at impl time discovered Round 1 missed the *external-constructor* case — `VjpOp::parents(&self) -> &[u32]` required external crates to store parent indices as `u32`, but `Var` at autograd.rs:110 has a private `u32` field with no public accessor, `impl From<Var> for u32`, or `Deref`. Verified via probe crate: external access fails `E0616: field 0 of struct Var is private`. **Resolution:** chassis refactored in one commit to move parents from the `VjpOp` trait to `Tape::push_custom(parents: &[Var], value, op)` — trait now traffics in `Var` (the typed handle) instead of leaking `u32`, parents can't drift out of sync with what was pushed, matches JAX's `custom_vjp` / PyTorch's `autograd.Function`. Zero existing external consumers; chassis commit `5f1cffba` lands before step-5 sim-soft work.
...
| SD-1 | `sim/L0/soft/Cargo.toml` | ~~Scaffold Cargo.toml omits §4's `[features]` table (`default = []`, `gpu-probe = ["dep:wgpu", "dep:bytemuck", "dep:pollster"]`) and the three optional deps.~~ **CLOSED by §9 step 7 GPU-probe commit** — Cargo.toml now carries `[features] default = []` + `gpu-probe = ["dep:wgpu", "dep:bytemuck", "dep:pollster"]` and three optional workspace-inherited deps (`wgpu`, `bytemuck`, `pollster`). Green-checklist items 2 + 4 (`cargo build --features gpu-probe` + `cargo test --features gpu-probe`) reachable. | Scaffold deferral — closed |
...
| SD-4 | `.github/workflows/quality-gate.yml:128-134` | CI `test` job's `cargo test --all-features -p <list>` hardcodes a crate list that does not include `sim-soft` (nor several other L0 crates — `sim-ml-chassis`, `sim-rl`, `sim-thermostat`, `sim-opt`, `sim-therm-env`). The `grade` job's `cargo xtask grade-all` covers clippy `--all-features` per crate (so I-6a's build graph is exercised), but per-crate `cargo test` inside `grade.rs:891-899` runs with **default features only** — so `gpu-probe` tests do not execute in any CI surface today. Step-7 green-checklist item 4 (`cargo test -p sim-soft --release --features gpu-probe`) is reachable locally only. Resolution in a followup platform-infra PR: (a) add `-p sim-soft` to `quality-gate.yml` test matrix (picks up `--all-features` which includes `gpu-probe`; probe's skip-clean branch handles headless runners); (b) optionally extend to the other omitted L0 crates if scoped that way. Non-blocking for step-7 which closes I-6b locally. | Scaffold deferral |
| SD-5 | `sim/L0/ml-chassis/Cargo.toml` → `sim-mjcf → mesh-io → zip → zstd-sys` | ~~`cargo tree -p sim-soft` surfaces `cc v1.2.57` as a build-dep through the transitive chain `sim-soft → sim-ml-chassis → sim-mjcf → mesh-io → zip → zstd → zstd-sys`.~~ **CLOSED** 2026-04-23 — verdict **(c) extractable**: the only production usage of `sim-mjcf` from chassis was three public stock task factories (`reaching_2dof` / `reaching_6dof` / `obstacle_reaching_6dof`) which had a natural home in `sim-rl::tasks` (where `sim-rl/src/lib.rs:43` was already re-exporting them). Chassis's touch surface was narrow: one function (`sim_mjcf::load_model(&str) -> Result<sim_core::Model, _>`), no types leaked through `lib.rs`. Refactor relocated the factories, promoted `sim-mjcf` to `[dependencies]` in `sim-rl`, demoted it to `[dev-dependencies]` in chassis (for `#[cfg(test)]` fixtures), and added a chassis-local `#[cfg(test)] mod test_stock_tasks` so `competition.rs` tests don't need a cyclic `sim-rl` dev-dep (which would resolve `TaskConfig` to a distinct type). Audit memo in `memory/project_ml_chassis_mjcf_dep_audit.md`. `cargo tree -p sim-soft -e normal` now shows **zero** `mjcf`/`mesh-io`/`zip`/`zstd-sys`/`cc`. §7 item 7 restored to literal pure-Rust claim. | Platform-infra audit, CLOSED |
| BF-7 | `sim/L0/soft/src/readout/skeleton_forward_map.rs` + `observable/basic.rs` + [Part 10 Ch 00 `RewardBreakdown`](../../../docs/studies/soft_body_architecture/src/100-optimization/00-forward.md) | `RewardBreakdown` has 4 fields per γ-lock: `pressure_uniformity`, `coverage`, `peak_bound`, `stiffness_bound`. On the skeleton's 1-tet scene with no contact, only `peak_bound` + `stiffness_bound` are structurally defined; `pressure_uniformity` + `coverage` are set to `f64::NAN` sentinels by `BasicObservable`. `score_with(&weights)` NaN-skips those fields so the composite reward scalar is finite. Per scope §11 S-1 (user-accepted) and §7 item 10, this gap is recorded rather than worked around. **Book-edit queue:** Part 10 Ch 00 should flag the sentinel pattern as expected for degenerate scenes, and either add a 2-tet + rigid-wall example that exercises all four fields or explicitly name the 1-tet degeneracy as out-of-scope for the `RewardBreakdown` contract. **Closed 2026-04-26** by Phase 1 PR (`feature/phase-1-bf7-bf8-preemptive`) — Part 10 Ch 00 now carries the NaN-sentinel paragraph naming "structurally undefined for this scene, not 'computation failed'" with downstream-consumer guidance for Ch 03 / Ch 05 / Part 11 Ch 04 §01. | Book-iteration finding — CLOSED |
| BF-8 | [Part 11 Ch 01 `00-core.md` `Solver::step`](../../../docs/studies/soft_body_architecture/src/110-crate/01-traits/00-core.md) | Book pins `Solver::step(..., theta: &Tensor<f64>, ...)`. The shipped skeleton uses `theta_var: Var` instead (scope §14 L437–438 — `CpuNewtonSolver::step` reads θ's current value via `tape.value_tensor(theta_var)` and later pushes `NewtonStepVjp` onto the tape with `theta_var` as parent via `push_custom`). The book signature pre-dates PR #213's `push_custom` chassis model — same root cause as BF-4/BF-6 (Differentiable::register_vjp / ift_adjoint). Skeleton follows scope §14's explicit commitment; book Ch 01 needs rewriting to take a `Var` handle so consumers can wire VJP parents without re-fetching parent indices out-of-band. **Closed 2026-04-26** by Phase 1 PR (`feature/phase-1-bf7-bf8-preemptive`) — book Ch 01 00-core.md now pins `theta_var: Var` on `step`, retains `theta: &Tensor<f64>` on `replay_step`, with rationale paragraph naming the tape-composing-vs-pure-function asymmetry. | Trait-signature / platform-API mismatch — CLOSED |
| BF-9 | [Part 5 Ch 00 `00-backward-euler.md`](../../../docs/studies/soft_body_architecture/src/50-time-integration/00-backward-euler.md) lines 68–91 + [`01-newton.md`](../../../docs/studies/soft_body_architecture/src/50-time-integration/00-backward-euler/01-newton.md) lines 61–84 | Same drift class as BF-8 plus deeper. Free-standing `pub fn step(tape: &mut Tape, ..., theta: &Tensor<f64>, ...)` pseudocode in both chapters pre-dates PR #213's `push_custom` chassis API; additionally calls `tape.record_ift_step(&step)` (book's pre-PR-#213 registry-key autograd model — same root cause as BF-4/BF-6). Identified during Phase 1 (BF-7+BF-8) cross-chapter audit 2026-04-26 per [`phase_1_bf7_bf8_preemptive_scope.md` §6 R-2](phase_1_bf7_bf8_preemptive_scope.md). Fix is a pseudocode-body rewrite (parameter + tape API + body), not a parameter swap. Skipped from Phase 1 PR per Decision B of that scope memo (cutting hard per `feedback_scope_creep_correction`); next drip-cadence pull when a future C1 phase touches these chapters. **Closed 2026-04-26** by Phase 2 PR (`feature/phase-2-multi-element-fem`) — Phase 2's production code IS the implementation of Part 5 Ch 00, making the rewrite cheap-and-on-path per Decision C of [`phase_2_multi_element_fem_scope.md`](phase_2_multi_element_fem_scope.md). Both chapters now use the post-PR-#213 chassis API (`Llt<usize, f64>` + `SymbolicLlt::try_new` + `Llt::try_new_with_symbolic` two-step + `theta_var: Var` + `tape.push_custom(&[theta_var], x_n.clone(), Box::new(NewtonStepVjp::new(factor)))`). Cross-chapter coherence verified via R-3 audit (parent + sub-leaf use byte-identical pseudocode + identical prose framing). Plus chapter-coherence prose edits at parent line 53/124 and sub-leaf line 48/98 (per `feedback_no_reflexive_defer`). | Pseudocode / platform-API mismatch — CLOSED |
| BF-10 | [Part 11 Ch 00 §08 `08-sdf-bridge.md`](../../../docs/studies/soft_body_architecture/src/110-crate/00-module-layout/08-sdf-bridge.md) Claim 1 + [Part 7 Ch 01 `01-tet-strategies.md`](../../../docs/studies/soft_body_architecture/src/70-sdf-pipeline/01-tet-strategies.md) Claim 2 | Pre-pivot framing of Phase A–I shipping fTetWild predated the 2026-04-27 BCC pivot (see [`phase_3_sdf_tet_bridge_scope.md` §0.5](phase_3_sdf_tet_bridge_scope.md)). Post-pivot reality is staged delivery: Phase G ships [Labelle-Shewchuk Isosurface Stuffing](../../../docs/studies/soft_body_architecture/src/appendices/00-references/03-diff-sim.md#labelle-shewchuk-2007) on a body-centered-cubic lattice as a placeholder mesher (theorem-bounded mesh quality on a sphere-SDF-only validation scene); Phase H ships fTetWild as a fidelity upgrade. Both chapters needed amending to reflect staged delivery + the Phase G placeholder's role as algorithmic foundation reusable by Phase H (BCC lattice + case-table reasoning + deterministic warp step + bipartite-lattice edge caching). **Closed 2026-04-27** by Phase 3 PR (`feature/phase-3-sdf-tet-bridge`) — 08-sdf-bridge.md amended at three locations (table row + Claim 1 prose block + Phase G deliverable bullet) to encode staged delivery and Phase G's two-of-six sub-area scope; 01-tet-strategies.md Claim 2 amended with "eventual" qualifier on the bold title plus a single staged-delivery sentence in the body. | Book-iteration finding — CLOSED |
| BF-12 | [Part 4 Ch 00 §00 `00-penalty.md`](../../../docs/studies/soft_body_architecture/src/40-contact/00-why-ipc/00-penalty.md) "What this sub-leaf commits the book to" + [Part 4 Ch 00 `00-why-ipc.md`](../../../docs/studies/soft_body_architecture/src/40-contact/00-why-ipc.md) parent header | Phase 5 (penalty rigid↔soft contact, one-way) substantively conflicts with §00 §00's *"`sim-soft` does not ship a penalty contact implementation, even as a baseline"* commitment: gameplan schedules penalty as a Phase H stepping-stone for rigid↔soft co-sim plumbing + first-time `ContactModel` Newton-hot-path wiring + Hertzian analytic-comparison gate. Reconciliation amends §00 §00's bullet to *"as a *production* baseline"* (preserving the "no production penalty" commitment while authorizing the Phase 5 stepping-stone scope); the parent header at §00 gets a forward-pointing one-liner naming the penalty implementation + IPC as the destination. Penalty's structural failure modes (active-set-boundary discontinuity, parameter sensitivity, oscillation pathology) stay valid; gradient optimization through penalty is restricted to the robustly-in-contact band (`sim/L0/soft/tests/contact_grad_hook.rs`); contact-pressure read into `RewardBreakdown` is best-effort with a documented overlap tolerance, not the precise observability IPC will provide at Phase H. **Closed 2026-04-29** by Phase 5 PR (`feature/phase-5-penalty-contact`) — see [`sim/L0/soft/src/contact/penalty.rs`](../../L0/soft/src/contact/penalty.rs). | Book-iteration finding — CLOSED |

BF-7 and BF-8 closed by Phase 1 PR 2026-04-26 (`feature/phase-1-bf7-bf8-preemptive`). BF-9 closed by Phase 2 PR 2026-04-26 (`feature/phase-2-multi-element-fem`) — see Decision C of [`phase_2_multi_element_fem_scope.md`](phase_2_multi_element_fem_scope.md). BF-10 closed by Phase 3 PR 2026-04-27 (`feature/phase-3-sdf-tet-bridge`) — see Decision N of [`phase_3_sdf_tet_bridge_scope.md`](phase_3_sdf_tet_bridge_scope.md). BF-12 closed by Phase 5 PR 2026-04-29 (`feature/phase-5-penalty-contact`) — see [`sim/L0/soft/src/contact/penalty.rs`](../../L0/soft/src/contact/penalty.rs). BF-1..BF-6 are not skeleton-blocking — the skeleton implements from the underlying math + the verified faer/chassis APIs + scope §14's override where book signatures pre-date the PR #213 chassis model. They're book-edit backlog for whenever Pass 2/3 touches these chapters. SD-1 closed at §9 step 7 (GPU-probe feature commit); SD-2 closed at §9 step 4 (Solver commit); SD-3 is a scope-memo correction, to be resolved by book-iteration pass per §11 S-10; SD-4 is a CI-infra deferral, to be resolved by a platform-infra followup PR adding `sim-soft` (and optionally other omitted L0 crates) to `quality-gate.yml` test matrix; SD-5 was closed 2026-04-23 by relocating stock task factories from `sim-ml-chassis` to `sim-rl::tasks` and moving `sim-mjcf` to chassis `[dev-dependencies]` (verdict (c) extractable; pure-Rust sim-soft graph is now literal).
