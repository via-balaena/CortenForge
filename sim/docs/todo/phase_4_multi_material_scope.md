...
- `Mesh` trait at `sim/L0/soft/src/mesh/mod.rs:57-80` — `n_tets`, `n_vertices`, `tet_vertices`, `positions`, `adjacency`, `quality`, `equals_structurally`. Phase 4 *adds* one method (`element_materials`) with **no default impl** (Decision in §9 — forces explicit migration so every mesh impl carries a real cache, not a fallback empty slice).
...
|---|---|---|
| Solver material storage | `sim/L0/soft/src/solver/backward_euler.rs:113` (struct field `material: M`) | Field deleted; replaced by reads from `mesh.element_materials()`. `M: Material` generic stays on the struct for now (keeps the type alias `CpuTet4NHSolver<Msh>` valid) but is unused for storage; followup audit may drop it post-Phase-4. |
| Solver `new()` signature | `sim/L0/soft/src/solver/backward_euler.rs:196-203` | First argument `material: M` becomes implicit (read from `mesh.element_materials()`). API break; all call sites update atomically (limited to test fixtures + `SoftScene::*` constructors). |
| Solver assembly hot points | `sim/L0/soft/src/solver/backward_euler.rs:751` (`first_piola`) and `:808` (`tangent`) | `self.material.first_piola(&f)` → `mesh.element_materials()[tet_id as usize].first_piola(&f)`. Same for `tangent`. The `tet_id` is already in scope at both sites. |
...
| **IV-4** | **SDF-driven region tagging is correct under refinement.** | 3-shell `LayeredScalarField` over concentric `SphereSdf`: every tet's `NeoHookean` instance matches the expected per-shell value at all centroids except a thin band near each shell boundary; misclassification count decreases monotonically as the mesher's `cell_size` halves. |
...
| **IV-6** | **Interface-tet flag is correct and refinement-monotone.** | `MaterialCache::interface_flags` populated by `|φ_blend(centroid)| < L_e` rule per Part 7 §02 §01. Test: refining a 3-shell mesh halves the interface-tet count to within ~25% (the rate is geometric, not exact, due to discrete sampling). |
...
|---|---|
| `sim/L0/soft/src/solver/backward_euler.rs` | Drop `material: M` field. `new()` no longer takes `material: M`. Lines `:751` and `:808`: replace `self.material.{first_piola,tangent}(&f)` with `mesh.element_materials()[tet_id as usize].{first_piola,tangent}(&f)`. Generic `M: Material` retained on the struct (Decision G); type aliases unchanged. |
| `sim/L0/soft/src/mesh/mod.rs` | Add `fn element_materials(&self) -> &[NeoHookean]` to the `Mesh` trait. Phase 4 monomorphizes against `NeoHookean` (Decision G). Phase H may generalize to `&[Box<dyn Material>]` when HGO joins; not done preemptively. |
...
6. **Commit 6: Solver assembly migration + IV-1 re-confirmation.** Drop `material: M` field from `CpuNewtonSolver`. `new()` no longer takes `material: M`. `:751` and `:808` read from `mesh.element_materials()[tet_id as usize]`. **Gate:** IV-1 still green (this is the structural moment).
...
- **`Mesh::element_materials` default impl** — `default_impl_returning_empty_slice` (forces all impls to opt in via deprecation warning) vs no-default (compile error forces immediate migration). Resolve at commit 4 time. Recommend no-default (forces explicit migration; cleaner). Bookkeeping cost ~5 lines per `Mesh` impl.
