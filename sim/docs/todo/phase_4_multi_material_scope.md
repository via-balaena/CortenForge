...
| `sim/L0/soft/tests/interface_band_flagging.rs` | **IV-6** — interface-tet flag fraction (`flagged / n_tets`) decreases monotonically under refinement; flag values are accurate (`|φ| < L_e`). Gate is on fraction, not absolute count: under cell-size halving in 3D the count grows as `1/h²` (band area / per-tet face area) while the fraction shrinks as `h`. |
