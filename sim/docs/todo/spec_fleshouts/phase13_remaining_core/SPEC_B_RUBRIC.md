...
| P4. Acceptance Criteria Rigor | **A** | 9 ACs covering early termination, force correctness, solver_stat, max_iters edge case, QCQP code review, Newton regression, warmstart review, golden flag checkpoint, PGS MuJoCo conformance. Each has concrete model/expected/field. Minor gap: AC1 lacks MuJoCo-verified expected iteration count (behavioral assertion only). |
...
**Overall: A+ (8 criteria) / A (1 criterion) — Rev 2**

The one A grade (P4) has a minor gap that doesn't affect implementability:
- P4: AC1 uses behavioral assertion rather than MuJoCo-verified iteration count (acceptable — iteration count is model-dependent)
...
|---|-----------|-----|-----------------|------------|----------|
| G1 | P4 | AC1 lacks MuJoCo-verified expected PGS iteration count | Initial grading | Acceptable: iteration count is model-dependent. Behavioral assertion (converges before max) is the correct conformance target. MuJoCo iteration count can be added as a supplementary data point but is not required for conformance. | Rev 1 |
