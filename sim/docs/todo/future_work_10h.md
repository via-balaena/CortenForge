...
| DT-65 | §6 | User sensor `dim` attribute not parsed — `User.dim()` returns 0. Also requires `sensor_intprm` array (`mjmodel.h:1213`, `nsensor × mjNSENS=3`) for user/plugin sensor integer params (separate from `sensor_history`; see Phase 6 Spec D rubric EGT-8). | Low | T1 |
