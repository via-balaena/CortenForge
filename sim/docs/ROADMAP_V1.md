...
| DT-175 | Cleanup | T1 | Thread margin into mesh collision helpers — `mesh_collide.rs` receives `margin` parameter but does not propagate it into sub-helper geometry queries. Identified during v1.0 cleanup (B4/T16). |
| DT-176 | Cleanup | T1 | Thread margin into SDF collision helpers — `sdf_collide.rs` receives `margin` parameter but does not propagate it into SDF distance queries. Identified during v1.0 cleanup (B4/T16). |
| DT-177 | Cleanup | T2 | URDF mesh asset support — `converter.rs` skips `<mesh>` geometry elements because URDF→MJCF conversion lacks asset declaration infrastructure. Identified during v1.0 cleanup (B4/T16). |
| DT-178 | Cleanup | T1 | Height field visualization — `sim-bevy` mesh conversion returns `None` for `CollisionShape::HeightField`. Requires grid→mesh tessellation. Identified during v1.0 cleanup (B4/T16). |
