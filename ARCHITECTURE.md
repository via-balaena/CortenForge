...
- **Presentation is quarantined:** everything Bevy lives in `cf-viewer`, `cf-bevy-common`,
  `sim-bevy`, `sim-bevy-soft`, `cf-mesh-paint`, and the `*-gui` tools. The spine never
  imports them.
- **~230 example / test crates** under `examples/**` are leaf *consumers* — allowed to
  depend on anything; they are out of scope for the tier rules.
...
- **The spine is clean:** none of the 14 core compute crates depend on Bevy or on a domain
