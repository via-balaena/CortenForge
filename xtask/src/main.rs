...
// has none, and while this logic lived only in build.rs a mutation survey found that
// gutting the hook-ownership check — and crossing the two hooks' filenames — passed
// the entire suite.
//
// ⚠ This used to carry `#[allow(dead_code)]`, explained by a paragraph saying the
// DECISIONS were called only from the build script. That stopped being true when
// `setup` began classifying before it writes: `title_of`, `classify` and `HookState`
// all have callers here now. Verified with `--force-warn dead_code` — nothing in this
// module is dead — so the attribute was suppressing nothing and the explanation had
