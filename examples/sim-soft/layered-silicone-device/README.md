...
Cavity asymmetry is visible: the offset 12-tri cube cavity at `(0.015, 0, 0)` produces an inner-ring distribution that is denser on the `+x` half of the body and the cavity-wall vertices on the cube's `+x` face show the highest displacement magnitudes (the cube extends out to `x = 0.040 m`, closer to the outer Dirichlet pin than the `-x` cavity-wall vertices at `x = -0.010 m`).
