...
- **(b) Decision J adjustment** — global default `κ` reduction. Out of V-3a authority (V-3 commit 9 has separate authority over defaults under Hertz geometry); would silently affect V-1 / V-3 / V-4 / V-5 / V-7.
...
Production scenes (V-1 commit 7 + V-3 commit 9 + V-4 / V-5 / V-7 commits 10 / 11 + rows 12 + 13) continue to use the default `(κ, d̂)` per scope memo Decision J; V-3a's override is local to this example only, never propagated upstream. **`KAPPA` stays at the `PENALTY_KAPPA_DEFAULT = 1e4` value** (no κ change, only `d̂`).
...
- **Bottom plate** (`Mesh3d` + `MeshMaterial3d`) — `8 m × 8 m` total (`4 × EDGE_LEN × RENDER_SCALE = 4 m` half-size) gray PBR quad at Bevy `y = -δ · RENDER_SCALE ≈ -0.005 m` (50 μm below the cube's rest bottom face — symmetric placement to the top plate, both `δ` away from their respective cube faces in the rest config; offset avoids depth-buffer z-fight with the coplanar cube bottom). **The bottom plate is purely visual** — physics has no penalty contact at the bottom face; the bottom face is BC-pinned in `BoundaryConditions::pinned_vertices`. The visualization shows two plates because the [`EXAMPLE_INVENTORY`][inv] row 14 framing is "soft cube between two RigidPlanes," and the BC-pin is the nearest-representable analog of a perfectly bonded rigid bottom plate (Phase 5 `BoundaryConditions` only models full-vertex Dirichlet); the bottom plate carries no penalty force in the physics.
- **Top plate** (`Mesh3d` + `MeshMaterial3d`) — same gray PBR quad at Bevy `y = (EDGE_LEN − DISPLACEMENT) · RENDER_SCALE ≈ 0.995 m` (physics z = `EDGE_LEN − DISPLACEMENT ≈ 9.95 mm`, just below the cube's rest top face by exactly `δ = 5e-5 m`). This is the actual physical `RigidPlane` — penalty contact occurs against this plate.
- **HUD** (`Text` + `Node`) — top-left, ASCII-only multi-line: per-refinement ε / F_R / [F_us, F_strain] / Cauchy ratio / E_eff / rel_pos_in_bounds. Reads 1:1 with the asserted scalars.
