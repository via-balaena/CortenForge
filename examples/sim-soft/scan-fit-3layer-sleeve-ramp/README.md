...
- **v?** — 8 mm intrusion (the user-target physical depth, structurally unreachable on this material model + geometry). Post-ship investigation (Dirichlet displacement-control spike, see [v2 spec memo's "Post-ship investigation" section][v2spec]) identified the wall as the **Neo-Hookean validity domain** (Phase 4 Decision Q fail-closed at `max_stretch_deviation < 1.0`), tripping at ~6.5 mm cavity-wall displacement when one tet's principal stretches reach `[2.06, 1.22, 0.073]`. faer LU fallback would NOT help — the validity gate is upstream of the linear solve. Three viable paths: (a) **wider-validity material model** — Mooney-Rivlin or higher-order hyperelastic, calibrated at multiple strain levels (aligns naturally with the project's Fork-B post-cast modulus calibration plan); (b) **local h-refinement** near the contact band so per-tet strain stays smaller; (c) **change the contact geometry** — lateral probe, smaller probe radius, or distributed traction to avoid the point of extreme local strain.
