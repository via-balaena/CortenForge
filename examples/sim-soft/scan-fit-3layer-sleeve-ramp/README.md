...
- **v?** — 8 mm intrusion (the user-target physical depth, structurally unreachable on this contact configuration). Two viable paths: (a) implement the documented faer LU fallback at `sim/L0/soft/src/solver/backward_euler.rs:597` so the engine can absorb the near-singular tangent regime around `max_disp ≈ 7 mm`; (b) change the contact geometry — probe placement (lateral instead of axial) or shape (smaller probe radius) to avoid the cavity-wall deformation pattern that hits the wall.
