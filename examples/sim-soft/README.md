...
| Example | Concept |
|---------|---------|
| [`stretch/stress-test`](stretch/stress-test/) | The sim-soft uniaxial-stretch surface as one validation superset on the canonical compressible NH baseline (`μ = 1e5`, `Λ = 4e5` Pa, `ν ≈ 0.4`), rows 4–6 folded — one domain → one stress-test, a deliberate solver → constitutive → assembly ladder — three modules: **`single_tet`** (row 4, Tier 1) — the arc's first FEM-running example, `SkeletonSolver::step` (backward-Euler Newton with `NeoHookean`, no contact) on `SoftScene::one_tet_cube` with `θ = 10 N` along `+ẑ` on `v_3` and `v_0..v_2` Dirichlet-pinned; converges in 3 iters to `dz ≈ 9.69e-4 m` (~1 % strain) — a demonstration gate self-gating only that the solve converged (Newton `iter_count < budget` + `residual < tol`); the physical correctness (Dirichlet-pin enforcement, `+ẑ` displacement sign, `F = I` axis alignment, byte-for-byte trajectory) is lib-owned (`solver_convergence.rs::stage_1_traction_converges` + IV-1), the frozen-bit-pattern self-pin stripped; **`neo_hookean`** (row 5) — NH force-stretch curve across a 12-point traction-free uniaxial sweep on a single Tet4 (`F = diag(λ, λ_t, λ_t)` with `λ_t` from a 1-D inner Newton on the traction-free transcendental). Observed-only demonstration: the self-gates read the **real** `Material::first_piola` / `Material::energy` output — inner-Newton convergence, traction-free `P_22`/`P_33`, `P_11` monotonicity + sign, and sweep-in-domain (`max\|σ−1\|` below the live `validity()` bound). The closed-form `first_piola`/`energy`-vs-analytic correctness is lib-owned (`neo_hookean.rs`); the analytic overlay + 48 captured-bit self-pins were stripped, so the JSON + optional `uv run plot.py` matplotlib 2×2 panel carry the real observed curve only; **`multi_element`** (row 6) — Phase 2 multi-element FEM assembly under uniform Dirichlet stretch on a 27-vertex / 48-tet hex grid (`HandBuiltTetMesh::uniform_block(2)`) at `λ = 1.20`, one interior vertex free (3 iters, residual ~1e-14 N). Keeps the assembly oracle: per-tet `F = diag(λ, 1, 1)` for every one of the 48 tets with a bounded `P_11` uniformity spread (`0.0` Pa) + validity-in-domain + the Dirichlet-pin bit-equal invariant (pinned DOFs left untouched by the solve); quasi-static via `cfg.density = 0`. The per-tet `P_11`/`P_22`/ψ go to the JSON as **observed** values — their closed-form correctness is lib-owned (`neo_hookean.rs` + `tests/uniaxial_fem_coupon.rs`), the 10 captured-bit self-pins stripped. JSON-only traces under `out/<module>/`. |
