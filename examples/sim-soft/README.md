...
- **PLY + JSON** for static math-pass-first artifacts (Tiers 1, 2,
  ...
  scalar to pre-select and the canonical visual. Some Tier 2 + 5
  rows (`neo-hookean-uniaxial`, `silicone-material-table`,
  `single-tet-stretch`, `multi-element-stretch`) ship JSON-only —
  the table or curve IS the artifact, no spatial render is
  meaningful (per
