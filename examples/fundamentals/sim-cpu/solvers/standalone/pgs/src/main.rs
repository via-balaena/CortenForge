//! PGS Solver — Projected Gauss-Seidel on a Two-Box Stack
//!
//! Two boxes stacked on a ground plane, solved with PGS (dual-space,
//! iterative). The HUD shows per-step iteration count and max contact
//! penetration. PGS is the workhorse solver — it always converges, but
//! needs more iterations than CG or Newton.
//!
//! Validates:
//! - Stack stable: `box_a` z-drift < 1mm over 5s
//! - PGS converges: avg iterations <= 50
//!
//! Run with: `cargo run -p example-solver-pgs --release`
...
const SOLVER_NAME: &str = "PGS";

// ── Bevy App ────────────────────────────────────────────────────────────────

fn main() {
    println!("=== CortenForge: {SOLVER_NAME} Solver ===");
    println!("  Projected Gauss-Seidel — dual-space iterative solver");
...
fn solver_diagnostics(
    ...
    // Accumulate stats every frame
    val.total_iter += data.solver_niter;
    val.steps += 1;

    let box_a_z = data.qpos[2];

    // After 0.5s, record rest position
    ...
    if harness.reported() && !val.reported {
        ...
        let checks = vec![
            ...
            Check {
                name: "PGS converges: avg iter <= 50",
                pass: avg_iter <= 50.0,
