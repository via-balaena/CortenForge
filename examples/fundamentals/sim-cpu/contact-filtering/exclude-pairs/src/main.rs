...
mod tests {
    ...
    ///
    /// `sphere_normal` rests on `plat_right`, while `sphere_excluded` passes
    /// through `plat_left` because the pair is in `<contact><exclude>`. The set
    /// equality asserts that absence.
    ///
    ...
    /// `<contact><exclude>` path: demonstrated failing by ignoring `contact_excludes`.
    ...
    fn the_settled_contact_set_matches_mujoco() {
        ...
        let want: Vec<String> = vec!["plat_right<->sphere_normal".to_owned()];
