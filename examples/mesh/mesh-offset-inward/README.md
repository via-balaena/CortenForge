...
```text
cargo run -p cf-viewer --release -- examples/mesh/mesh-offset-inward/out/before.ply
cargo run -p cf-viewer --release -- examples/mesh/mesh-offset-inward/out/after_flipped.ply
cargo run -p cf-viewer --release -- examples/mesh/mesh-offset-inward/out/after.ply
```
...
- **`out/after.ply`** — *the platform-truth artifact*, geometrically
  ...
  viewers configured for two-sided lighting without culling (cf-view's
