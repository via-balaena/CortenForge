...
fn main() -> Result<()> {
    ...
    // Manual orbit-camera wiring (instead of `OrbitCameraPlugin`) so
    // we can gate `orbit_camera_input` on `orbit_camera_should_run` —
    // egui-aware run condition that suppresses scene drags while a
    // side-panel slider / button is consuming pointer input. The
    // `update_orbit_camera` transform application runs unconditionally
    // (cheap; just projects the current OrbitCamera state onto
    // Transform) so the scene stays steady when input is suppressed.
    .add_systems(
        Update,
        (
            orbit_camera_input.run_if(orbit_camera_should_run),
    ...
    app.add_systems(Startup, setup_scene)
        ...
        .add_systems(
            ...
            (
                scalar_and_colormap_panel,
                sequence_info_panel,
                // Must run AFTER the panels so it sees the egui-wants-
                // pointer state generated by their widgets. Powers the
                // orbit-camera-input gate next frame.
                record_egui_pointer_capture,
            )
                .chain(),
        )
        .run();
...
/// Resource that records "egui wants the pointer this frame" so the
/// orbit camera input system can be gated on it via `run_if`. Bevy's
/// `run_if` condition systems must take read-only params, so we
/// can't read `EguiContexts::ctx_mut()` (which mutates) directly from
/// the condition; instead each egui side-panel system writes this
/// resource at the end of its UI block, and the orbit camera reads
/// it next frame.
///
/// One-frame lag is imperceptible at 60+ fps. The default
/// (`captured = false`) lets the orbit camera respond normally on
/// the very first frame before any panel has run.
#[derive(Resource, Debug, Default)]
struct EguiPointerCaptured {
    captured: bool,
}

/// Run-condition system for the orbit camera input. Returns `true`
/// when egui is NOT consuming pointer input (so the orbit camera
/// should respond to mouse drags / scrolls), and `false` when egui
/// IS consuming pointer input (so the orbit camera should hold
/// still while the user drags a slider, clicks a button, etc).
///
/// Without this gate, dragging an egui slider also rotates the
/// scene behind it because `orbit_camera_input` reads
/// `ButtonInput<MouseButton>` + `AccumulatedMouseMotion`
/// unconditionally — bevy_egui captures the egui side of the input
/// but doesn't suppress the underlying Bevy mouse-input resources.
fn orbit_camera_should_run(capture: Res<EguiPointerCaptured>) -> bool {
    !capture.captured
}

/// Companion system to [`orbit_camera_should_run`] — runs in
/// `EguiPrimaryContextPass` AFTER the side-panel systems and writes
/// the consolidated "egui wants pointer this frame" state into the
/// [`EguiPointerCaptured`] resource. Decouples the run-condition's
/// read-only contract from `EguiContexts::ctx_mut()`'s mutable
/// access.
#[allow(clippy::needless_pass_by_value)] // Bevy systems take resources by value
fn record_egui_pointer_capture(
    mut contexts: bevy_egui::EguiContexts,
    mut capture: ResMut<EguiPointerCaptured>,
) {
    let Ok(ctx) = contexts.ctx_mut() else {
        return;
    };
    capture.captured = ctx.wants_pointer_input() || ctx.is_pointer_over_area();
}
...
fn run_assembly_mode(cli: &Cli, stls: &[std::path::PathBuf]) -> Result<()> {
    ...
    .add_systems(
        EguiPrimaryContextPass,
        // `record_egui_pointer_capture` runs AFTER the panel so it
        // sees the egui-wants-pointer state that the panel widgets
        // generated. Powers the orbit-camera-input gate next frame.
        (assembly_piece_panel, record_egui_pointer_capture).chain(),
    )
    .run();
